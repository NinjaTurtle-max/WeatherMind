...
- 테스트 실측(**2026-08-10**) **backend 4115 passed·8 skipped·1 xfailed** ·
  **ai-worker 294**(의존 전체 설치 시) · **celery 16** · 프론트 `package.json` `test:*`
  ⚠️ backend가 2275 → 4115로 뛴 것은 새 테스트를 그만큼 쓴 것이 아니라 **시드가
  272 → 1000건이 되면서 문항별 parametrize가 함께 늘어난 몫이 크다**. 이 숫자를
  "테스트 커버리지가 두 배가 됐다"로 읽지 말 것.
...
- **문항 1,015건**(**2026-08-14 재실측** — 8/12에 1,012건, staging board 승격 +3) · 유형 7종
  `multiple_choice 310 · cloze 158 · short_answer 155 · match 125 · ordering 111 · slider 107 · board 49`
  · 개념 태그 **14종**(개념 문서 태그 집합과 완전 일치)
  ⚠️ **이 절이 또 낡아 있었다 — 세 번째다.** 8/14에 바뀐 것은 **board +3**뿐인데
  유형 표의 `multiple_choice`·`cloze`·`short_answer`가 **이미 어긋나 있었다**(적힌 값
  316·156·151 ↔ 실측 310·158·155). 밴드·단계 표도 같았다. **내 변경분만 고치면 남은
  거짓이 그대로 남는다**는 것이 이번에 드러났고, 그래서 **전 행을 다시 셌다.**
...
- **지식 단계 10칸**(6→10 확장) — **2026-08-14 재실측** `1:99 2:101 3:99 4:109 5:102
  6:101 7:98 8:101 9:101 10:104`. **10칸 전건 98건 이상**이라 어느 단계로 배정돼도
