...
- **커리큘럼**(`curriculum.py`): 유닛 트리(섹션→유닛), `unit_order`·`prereq` 선행 잠금,
  ...
  **다과정**(R11): `courses` 테이블 + `GET /courses`·`?course=` — `units.course_id`
  NULL은 기본 코스(weather) 취급이라 완전 하위 호환. **basic-science 유닛 시드 완료**(R13 — "빈 트리"는
  낡은 기술이다). θ는 코스를 가로질러 개념 태그 단위.
  ⚠️ 이 자리에 **"3섹션 8유닛"**이 적혀 있었고 **거짓이 됐다**(2026-08-18 실측 —
  basic-science **99유닛** · 전체 237 = weather 138 + basic-science 99. 소유자는
  `database/seed/units.json`). 같은 단락 아래가 *"유닛 수를 여기 적지 않는다"*고
  적어 놓았는데 그 위에서 스스로 적고 있었다 — 그래서 수를 지우고 소유자만 남긴다.
  ⚠️ 대장 MT 표도 이 값을 **"기초과학 유닛 0건"**으로 적어 왔고, 그 때문에 MT-32의
  미반영 사유가 「그릇이 비어서」로 오독됐다. 진짜 사유는 **참고 설계가 없어서**다
  (2026-08-18 정정 — 대장 소유 세션 실측).
...
- ✅ **G1 앞의 선행 2건은 착지했다**(2026-08-09 확인 — 종전 "⚠️ 선행되어야 한다"는
  ...
  · **`rag-feedback` 상시 과금** — CO-I-1 배선으로 **사람 저작 해설이 LLM보다 앞**에
    온다. "board 외 전 유형에서 매 답안 1콜"은 더 이상 참이 아니다. ⚠️ 이 줄은
    **두 번 낡았다**: 「193건」은 저작 전 값이라 아래 실측(909건)과 어긋난 채였고,
    「해설 없는 45건」도 `ba038f0`(2026-08-13)이 전건 저작해 0이 됐다. 건수를 다시
    적지 않는다(§0-2) — **소유자는 아래 「콘텐츠 실측」 한 곳**이고, 남는 LLM 호출은
    「해설이 비어 있는 문항 + 생성 문항」이라는 **조건**으로만 적는다.
...
- ⚠️ **"1,000건 통과"가 "1,000건 검증"은 아니다.** 전건이 `lint_seed_items`를 통과했지만
  ...
  **채점 결함 2건**(오독이 정답 처리 · 맞는 답이 오답 처리)이 lint 초록 상태에서
  ...
  내려보낸다. 남는 LLM 호출은 **생성 문항**뿐이다(뱅크 몫은 위 실측대로 0).
- **커리큘럼 규모를 여기 적지 않는다**(§0-2) — 소유자는 `database/seed/units.json`과
  `curriculum_service.SECTION_ORDER`이고 정합은 `test_section_name_parity.py`가 문다.
  ⚠️ 종전 "2코스 · 8섹션 · 24유닛"은 **CO-G1 순환식 전환(2026-08-12) 전 값**이라,
  같은 파일 위쪽 「커리큘럼」 줄(13섹션 237유닛)과 **정면으로 어긋난 채 남아 있었다**.
  한 문서 안에서 같은 수치가 두 값을 갖는 것이 §0을 만든 이유다.
