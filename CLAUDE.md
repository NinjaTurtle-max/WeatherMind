...
- **다음 = R10** — 실행 계획 확정: `docs/team/SPRINT_R10_01.md`
  (근거: `docs/Observation_Report_03_R10_UX_Field_Test.md` 실사용 필드 테스트).
  **클라이언트 결정(2026-08-01)**: 구름은 **오답에만 1 소모**(정답·재제출·배치고사
  무소모) + **문항 진입 전 잔량 부족 차단**(진행 중 세션은 끝까지 보장) ·
  WebGL은 **raw 자체 구현**(three.js 불채택) · 범위는 **단면 패널 + 지도 오버레이**.
  R10-B(뱅크 확장)는 규모·주체 결정 대기로 스프린트 범위 밖.
  우선순위 전체:
