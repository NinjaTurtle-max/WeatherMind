...
5. 표현 톤은 teen(한다체) **한 벌만** 쓴다 — 평서는 "~한다", 질문은 "~인가/~하는가"로
   ...
   ⚠️ **knowledge_level이 1이나 2(초등)면 이 규칙 대신 아래를 따른다** — 어미 치환기가
   아직 없어 규칙 5의 한다체가 초등 학습자에게 그대로 나가고 있었다(2026-08-20 실측).
   초등 문항은 **처음부터 해요체 + 그림책 나레이션**으로 저작한다:
   · 문장 전체(질문·선지·정답 해설)에 의성어·의태어를 자연스럽게 섞는다
     (슝~ · 뭉게뭉게 · 후드득 · 쨍쨍 · 살랑살랑 등 — 상황에 맞게 새로 만들어도 된다).
   · 특정 용어 89개만 부드럽게 바꾸는 게 아니라 **문장 전체의 말투**를 그림책처럼 쓴다 —
     한 문장 한 개념, 눈에 보이고 몸으로 느껴지는 표현(뜨거워요·차가워요·세게 불어요)
     위주로, 국어사전식 정의문("~하는 현상이다")은 쓰지 않는다.
   · 그래도 규칙 4(단계별 도입 어휘 상한)는 그대로 지킨다 — 말투만 낮추고 개념 자체를
     끌어올리지 않는다.
...
#
# ── 🔴 어미 치환기가 실재하지 않아 해요체로 저작했다 (2026-08-20) ─────────────
# 스펙 03 §2 규칙 5와 프롬프트 규칙 5는 "본문은 teen(한다체) 한 벌, child/adult
# 어미는 **런타임 치환의 몫**"을 전제한다. 그런데 **그 몫을 아무도 하지 않는다** —
# `weatherbrain_service.effective_tone`은 톤을 *판정해 표시에 실어 보낼 뿐*이고,
# 문항 텍스트를 다시 쓰는 코드는 backend·frontend 통틀어 0건이다(2026-08-20 실측).
# 그래서 한다체로 저작하면 그 문자열이 **초등 학습자에게 한다체 그대로 나간다**.
#
# 판정(2026-08-20 클라이언트): 없는 치환기를 전제한 규칙 정합보다 실제 학습자가
# 우선이다 ⇒ **초등에게 서빙되는 문항은 해요체로 저작한다**. 대상은 (a) 앞 7건
# (기상 6태그 — 본문이 이미 해요체였는데 선지만 한다체로 섞여 있었다) (b) 뒤 8건
# 중 `knowledge_level == 2`인 3건(temperature_heat·phase_change·flood_response) —
# 생성분 적재 경로가 `knowledge_level`로 밴드를 파생시키므로 이 셋은 `elementary`
# 밴드 뱅크 문항으로 **영구 적재**된다. 나머지 5건(kl 3~5)은 teen 한다체 그대로다.
#
# ⚠️ **규칙 5 자체는 고치지 않았다**(docs/specs/03·12 무접촉). 이 뱅크는 규칙과
# 어긋난 채로 있고, 그 이유가 이 주석이다. **어미 치환기가 실제로 생기면 이 결정을
# 다시 볼 것** — 그때는 한 벌(teen) 저작 + 런타임 치환이 옳다.
#
# `correct_answer`는 톤 불변 필드다. 단 객관식에서 정답이 선지 문자열의 복사본일
# 때는 채점이 문자열 동일성이라 **선지와 함께** 고쳐야 한다(위 3건이 그 경우다).
#
...
FALLBACK_QUESTIONS: list[dict] = [
    {
        ...
        "options": ["맑고 건조해요", "흐리고 비가 오기 쉬워요", "기온이 급격히 떨어져요", "바람이 전혀 불지 않아요"],
        "correct_answer": "흐리고 비가 오기 쉬워요",
    },
    {
        "concept_tag": "pressure_front",
    ...
    # ── 기초과학 6태그 (2026-08-10 신규 · teen 톤 / kl2 2건은 해요체) ───────
    {
        "concept_tag": "temperature_heat",
        "knowledge_level": 2,  # 측정·관찰로 규칙성을 찾는 구간. '비열'(3)은 쓰지 않는다
        "question_type": "multiple_choice",
        "question_text": "한낮 바닷가에서 모래는 뜨거운데 바닷물은 미지근해요. 같은 햇빛을 받은 두 곳의 온도 변화를 바르게 말한 것은 무엇일까요?",
        "options": [
            "모래가 물보다 빨리 뜨거워져요",
            "물이 모래보다 빨리 뜨거워져요",
            "모래와 물이 똑같이 뜨거워져요",
        ...
        "correct_answer": "모래가 물보다 빨리 뜨거워져요",
    },
    {
    ...
    },
    {
        ...
        "question_type": "cloze",
        "question_text": "이른 아침 풀잎에 물방울이 맺혀요. 공기 중의 수증기가 차가운 물체에 닿아 액체로 바뀌는 이 현상을 ___이라고 해요.",
        "correct_answer": "응결",
    },
    {
    ...
    # ── 재난 2태그 (2026-08-10 신규 · teen 톤 / kl2 1건은 해요체) ──────────
    {
    ...
    },
    {
        ...
        "question_type": "multiple_choice",
        "question_text": "많은 비로 지하 주차장에 물이 차오를 때 가장 먼저 해야 하는 행동은 무엇일까요?",
        "options": [
            "차를 두고 즉시 밖으로 대피해요",
            "차를 옮기러 지하로 내려가요",
            "물이 빠질 때까지 차 안에서 기다려요",
            "주차장 배수구를 손으로 막아요",
        ],
        "correct_answer": "차를 두고 즉시 밖으로 대피해요",
    },
]
