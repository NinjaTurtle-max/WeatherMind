...
1. **찾을 것이 이미 확정돼 있었다.** 쿼리의 `concept_tag`는 호출부(`/answer`)가
   ...
   그 문서를 다시 찾고 있었다.
2. **코퍼스가 8KB다.** `climate_concepts.json` = 35항목 · 12개 태그. 태그당
   2~6항목이라 그 태그 전부를 넣어도 top_k=3보다 크지 않다. 색인·근사이웃탐색이
   푸는 문제(후보가 너무 많다)가 여기엔 없다.
...
**태그에 해당하는 개념 문서가 아예 없는 경우**(실측 본시드 237문항 중 15건 —
`flood_response`·`wildfire_weather`). 이때는 빈 컨텍스트를 넣지 않고
