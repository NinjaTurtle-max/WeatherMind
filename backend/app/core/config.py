...
class Settings(BaseSettings):
    ...
    # 분모 946의 출처(**2026-08-14 재실측**: 시드 **1,015**건 − board **49**건 = 비board
    # 966건 — board 승격 +3이 총량 +3과 상쇄돼 **분모는 안 바뀐다**. 8/12 값은
    # 1,012 − 46이었고 결과가 같다),
    ...
    #      **2026-08-14 재실측** 시드는 **1,015**건이고 board **49**건이며, 이 블록들이
    #      뽑는 풀은 비실황 **995**건이라 실제 비중은 **49/995 = 4.9%**다(8/12에는
    #      46/992 = 4.6%였다 — 승격 3건이 비중을 0.3%p 올렸을 뿐 판단은 그대로다).
    #      일반 블록 9칸에 적용한 기대값은 **0.44** — 상한 2는 사실상 아무것도 막지
    #      않는 값이 돼 있었다.
