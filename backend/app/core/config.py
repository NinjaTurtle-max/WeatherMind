...
class Settings(BaseSettings):
    ...
    # 분모 946의 출처(**2026-08-18 재실측**: 시드 **1,019**건 − board **53**건 = 비board
    # 966건 — **board가 늘 때 총계와 board 수를 함께 올리므로 분모는 안 바뀐다**.
    # 8/14 값은 1,018 − 52, 8/12 값은 1,012 − 46이었고 셋 다 결과가 같다.
    ...
    #      **2026-08-18 재실측(MT-19 판독 보드 반영)** 시드는 **1,019**건이고 board
    #      **53**건이며, 이 블록들이 뽑는 풀은 비실황 **999**건이라 실제 비중은
    #      **53/999 = 5.3%**다(8/18 아침 52/998 = 5.2% · 8/12 46/992 = 4.6% —
    #      board 저작이 비중을 조금씩 올릴 뿐 판단은 그대로다).
    #      일반 블록 9칸에 적용한 기대값은 **0.48** — 상한 2는 사실상 아무것도 막지
    #      않는 값이 돼 있었다.
