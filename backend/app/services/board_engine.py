...
def select_feedback(
    ...
    ⚠️ **해설이 없는 판은 규칙 설명만 나간다** — 이어 붙일 것이 없으면 종전과
    **같은 문자열**을 돌려야 한다. 그 회귀 0이 이 변경의 조건이다. 이 분기는 지금
    **닫혀 있다**(2026-08-20 실측: board **64판 중 해설 보유 64** — 종전 26에서
    37판을 저작해 채웠다). 그래도 분기를 남기는 이유는 새 판이 해설 없이 들어올 수
    있기 때문이다.
    ⚠️ **이 수를 기억으로 적지 말 것 — 세어서 적을 것.** 소유자는
    `database/seed/content_items.json`이고 세는 방법은 다음 한 줄이다:
    `python3 -c "import json;i=json.load(open('database/seed/content_items.json'));\
b=[x for x in i if x.get('question_type')=='board'];\
print(sum(1 for x in b if (x['template_json'].get('explanation_hint') or '').strip()),len(b))"`
