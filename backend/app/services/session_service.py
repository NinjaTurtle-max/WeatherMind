...
# `build_pool_query`의 |b−θ| 정렬로는 못 자른다. 시드 board **49건**(2026-08-14
# 재실측 — 승격 +3)의 밴드별 최대치(**middle_high 33**)를 덮는 값이라 사실상 전건을
# 보고 고른다. ⚠️ **여유가 7건으로 줄었다** — middle_high board가 40을 넘으면 이
# 상한이 실제로 자르기 시작하고, 그때부터 「전건을 보고 고른다」가 거짓이 된다.
BOARD_POOL_LIMIT = 40
