...
class TestBoardSeedShape:
    ...
    **파생과 무관한 시드 실측**뿐이라 이름을 그렇게 고쳤다.
    ...
    def test_시드_board_개수_고정(self):
        ...
        # MT-18 잔여 4판(2026-08-21 병합): board **64 → 68**(`board_order` 65~68 ·
        # kl8 2 · kl10 2). 엔진·규칙·현상 어휘 **0건 변경** — 기존 문법으로 표현됐다.
        assert len(self._seed_boards()) == 68
    ...
    def test_시드_board_지식_단계_분포_고정(self):
        ...
        **어느 칸으로 늘었는지**를 못 본다. 그것이 종전 계약의 값이었고 실제로
        ...
        # MT-18 잔여 4판(2026-08-21 병합): **kl8 4 → 6 · kl10 3 → 5.** 나머지 여덟 칸은
        # 그대로다 — 네 판이 전문가 축(kl8 2 · kl10 2)에만 붙었다는 것이 이 값의 내용이고,
        # 그래서 **kl 4의 26판 쏠림은 이 증보로 안 풀렸다**(전문가 축을 늘린 것이지
        # 쏠림을 고친 것이 아니다). 위 독스트링의 「41판 ↔ 15판」 갈림은 여전히 유효하다.
        assert dict(dist) == {
            1: 4, 2: 4, 3: 7, 4: 26, 5: 5, 6: 2, 7: 4, 8: 6, 9: 5, 10: 5
