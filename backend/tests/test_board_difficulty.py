...
class TestBoardSeedShape:
    ...
    **파생과 무관한 시드 실측**뿐이라 이름을 그렇게 고쳤다.
    ...
    # ⇒ 새 축의 단계 분포 핀이 있는지는 **리드에게 확인 요청**했다(내 소유 밖).
...
#
# 🔴 ⚠️ **그러나 브리핑의 전제 하나가 틀렸다 — 대체자도 함께 죽는다.**
# 브리핑은 *"B조가 `board_difficulty_samples`로 **새 축 규칙**을 대조하게 만들었다"*
# 고 적었으나, `:772`가 임포트하는 것은 **`board_difficulty`(파생축)**다. 즉 B조
# 항목은 새 축으로 갈아탄 것이 아니라 **같은 파생축을 더 강하게 문** 것이다.
# ⇒ 파생 함수를 지우면 B조 항목도 ImportError로 무너진다. 그 파일은 내 소유가
# 아니라 고치지 않고 **보고했다.** 「대체자가 있다」와 「대체자가 산다」는 다르다.
#
# 🔴 ⚠️ **더 큰 구멍: 목이 아직 새 축으로 갈아타지 않았다.**
# `frontend/mock/apiMockPlugin.js`는 여전히 `BOARD_BAND_MAX_DIFFICULTY`로 잠그고
# `boardPuzzlePayload`가 `difficulty`를 싣는다(서버는 `knowledge_level`). 목↔서버는
# **지금 이미 갈라져 있고**, 그 갈림을 보던 계약이 이 철거로 전멸한다.
# 목은 내 소유가 아니라 손대지 않고 보고했다.
