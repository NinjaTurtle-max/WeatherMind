...
## 🔴 2026-08-20 — **계약이 목의 설계를 막고 있었다**(정규식 열기)

`_fn_body`가 `function <name>()` — **인자 없는 형태만** 찾았다. 그래서 목이 잠금
규칙을 **순수 함수로 열면**(인자를 받으면) 이 계약이 함수를 **아예 못 찾아** 빨개졌고,
목 담당이 그 벽 때문에 `unlockedBoardIds` 순수 함수 전환과 `board_unlock_samples`
노출을 **되돌렸다**(목 파일의 그 선언부 주석과 `__mockPolicy()` 꼬리 주석이 소유).
⇒ 정규식을 `_FN_ARGS`(`\\((?:[^)]*)\\)`)로 열었다. **잡는 대상이 늘 뿐 줄어드는 것이
없으므로** 변이 보존 기준으로 **조이기**다 — 그 성질은 문서가 아니라
`TestParityInstrument::test_연_인자_정규식이_좁힌_것의_진상위집합이다`가 소유한다
(수를 적지 않는 이유: 처음 쟀을 때 `29 → 56`이던 것이 하루 만에 `29 → 57`이 됐다).

🔴 **열기만으로는 오늘 대조가 한 건도 안 늘었다** — 목의 두 대상
(`lockedBoardTiers`·`unlockedBoardIds`)이 마침 둘 다 인자가 없어 종전 정규식으로도
잡혔기 때문이다. 값은 **막고 있던 벽을 치운 것**이고(목 담당이 되돌린 순수 함수
전환과 `board_unlock_samples` 노출이 곧바로 가능해진다), 그래서 **되돌림이 조용하지
않도록** 두 자리를 함께 뒀다: `_fn_body`의 **본문 도달 자기 단정**과 위 진상위집합
단정이다. 그 침묵이 바로 이 벽이 오래 산 이유다.
"""
...
#: 함수 선언의 **인자 목록** — 열려 있어야 한다(인자 없는 형태만 보면 목이 규칙을
#: 순수 함수로 여는 순간 계약이 함수를 아예 못 찾는다). 상수로 뽑아 둔 이유는
#: **되돌림이 울게** 하기 위해서다: `TestParityInstrument`가 이 상수를 읽어
#: 「연 것이 좁힌 것의 **진상위집합**인가」를 문다. 다시 `\(\)`로 좁히면 거기가 운다.
_FN_ARGS = r"\((?:[^)]*)\)"
#: 종전(좁은) 모양 — 역검증·대조용으로만 쓴다. 이것으로 돌아가면 계약이 조용해진다.
_FN_ARGS_NARROW = r"\(\)"
...
def _fn_body(src: str, decl: str) -> str:
    ...
    🔴 **인자 목록을 받는 선언도 찾는다**(2026-08-20 — 어드바이저·PM 판정).

    종전 정규식은 `\\(\\)` — **인자 없는 형태만** 찾았다. 그래서 목이 이 규칙을
    **순수 함수로 여는 순간**(인자를 받는 순간) 이 계약은 함수를 **아예 못 찾아**
    빨개졌다. 실제로 그 벽 때문에 목 담당이 `unlockedBoardIds`를 열려다 되돌렸고,
    행동 대조를 못 붙인 채 소스 계약으로만 물렸다 — 경위는 목
    (`frontend/mock/apiMockPlugin.js`)의 **그 함수 선언부 주석**과 `__mockPolicy()`
    꼬리 주석이 소유한다(*"행동 대조는 그 정규식이 인자를 받게 넓혀질 때 이 자리에
    붙인다"*).

    ⚠️ **이것은 완화가 아니라 조이기다.** 판정 기준은 이 파일이 §5.27-k에서 이미
    쓴 **변이 보존**이다 — 고친 뒤 **잡는 변이가 줄어드는가**를 본다. 여는 쪽은
    잡는 대상이 늘 뿐 줄어드는 것이 없다.

    ⚠️ **그 「늘어난 수」를 여기 적지 않는다**(CLAUDE.md §0-2). 하루 만에 드리프트한
    값이기 때문이다 — 처음 쟀을 때 `29 → 56`(선언 57개)이었는데 같은 트리에서
    `applyReseedUnmeasured` 하나가 붙어 **다음 날 `29 → 57`(선언 58개)**이 됐다.
    **세는 방법만 남긴다**: 목의 `^function (\\w+)\\(` 선언 전체에 대해
    `_FN_ARGS`로 찾아지는 집합과 `_FN_ARGS_NARROW`로 찾아지는 집합을 비교한다.
    그리고 그 비교는 사람이 아니라
    `TestParityInstrument::test_연_인자_정규식이_좁힌_것의_진상위집합이다`가 한다.

    ⚠️ `[^)]*`라 **인자 기본값 안에 괄호가 있으면 못 찾는다**
    (`function reviewQueuePayload(nowMs = Date.now())` — 목에서 그 형태 1개).
    `.*?`로 더 넓히지 **않는다**: 본문 없는 이름(주석 속 `function foo`)에서
    캡처가 파일 끝의 아무 `) {`까지 흘러 뒤 단정이 **공허하게 초록**이 된다 —
    이 파일이 이미 한 번 밟은 형태다(머리말 ⓑ).

    🔴 **캡처가 본문에 닿았는지 스스로 단정한다**(`test_층_추출_규칙이_같다` ⓒ와
    같은 형태). 정규식을 다시 좁히거나 인자 캡처를 본문으로 잘못 돌려주면
    호출하는 **모든** 단정이 볼 대상을 잃는다. 인자 목록을 `(?:...)`로 두는 것도
    그 때문이다 — group(1)은 언제나 **본문**이어야 한다.
    """
    m = re.search(
        rf"function {re.escape(decl)}{_FN_ARGS}\s*\{{(.*?)\n\}}", src, re.S
    )
    assert m, f"목에서 {decl}를 못 찾았다 — 이름이 바뀌었나(이 계약을 갱신할 것)"
    body = _js_code(m.group(1))
    assert "return" in body, (
        f"`{decl}` 캡처가 함수 본문에 못 닿았다 — 호출하는 단정들이 볼 대상이 없어 "
        f"공허해진다(정규식을 좁히거나 인자 캡처를 본문으로 돌려주면 이 형태가 "
        f"된다). 캡처: {body.strip()!r}"
    )
    return body
...
class TestParityInstrument:
    """🔴 **계측기 자신을 무는 자리**(2026-08-20 신설).

    이 파일은 목 소스를 **정규식으로 파싱**한다. 그래서 정규식이 조용히 좁아지면
    계약이 **아무것도 안 잡는 상태**가 되는데 **초록**이다 — 머리말 ⓑ가 그 실례를
    적어 뒀다(`(.*?);`가 첫 세미콜론에서 끊겨 단정 2건이 공허했다).

    🔴 **오늘 발견**: `_fn_body` 정규식을 열기 **전에는, 다시 좁혀도 아무것도 울지
    않았다** — 목의 두 대상(`lockedBoardTiers`·`unlockedBoardIds`)이 마침 둘 다
    인자가 없어 `\\(\\)`로도 잡혔기 때문이다. 즉 **되돌림이 조용했다.** 그래서
    되돌림이 울게 만드는 단정을 여기 둔다. 이것이 없으면 다음 사람이 "인자 없는
    형태만 보면 충분하다"며 좁혀도 CI가 초록이고, **목이 규칙을 순수 함수로 여는
    날에야** 빨개진다 — 그때 고쳐지는 것은 계약이 아니라 **목 코드**다(실제로
    2026-08-20에 목 담당이 그 이유로 순수 함수 전환을 되돌렸다).
    """

    def test_계측기가_인자를_받는_선언도_찾는다(self, mock_src: str):
        """목이 규칙을 **순수 함수로 열어도** 이 계약이 계속 문다.

        목 소스를 **메모리에서** 변이시켜(파일은 건드리지 않는다) 인자를 받는
        선언으로 바꾼 뒤, `_fn_body`가 **같은 본문**을 돌려주는지 본다. 본문 동일성이
        ...
        assert _fn_body(broken, "unlockedBoardIds") == _fn_body(
            ...
            "`_fn_body`가 인자를 받는 선언에서 다른 것을 잡는다 — 정규식이 좁아졌거나"
            " 인자 캡처를 본문으로 돌려주고 있다. 그 상태면 목이 규칙을 순수 함수로 "
            "여는 순간 이 파일의 구조 단정 전부가 함수를 못 찾거나 공허해진다"
        )

    def test_연_인자_정규식이_좁힌_것의_진상위집합이다(self, mock_src: str):
        """🔴 **「연 것이 조이기다」를 값이 아니라 성질로 못박는다.**

        정규식을 연 근거는 *"잡는 대상이 늘 뿐 줄어드는 것이 없다"*였다. 그 근거를
        **문서가 아니라 여기가** 소유한다 — 처음 쟀을 때 `29 → 56`이던 값이 하루
        만에 `29 → 57`이 됐으므로(목에 함수 하나가 붙었다) **수를 적으면 낡는다.**
        대신 두 성질을 문다:

          ⑴ **진상위집합** — 연 정규식이 찾는 집합이 좁은 정규식이 찾는 집합을
             **통째로 포함**하고(잃은 것이 없다), **더 많다**(늘었다).
          ⑵ 그래서 `_FN_ARGS`를 다시 `\\(\\)`로 좁히면 두 집합이 **같아져** 여기가
             운다. 이 파일에서 **되돌림이 우는 유일한 자리**다 — 목의 두 대상이
             지금 마침 둘 다 인자가 없어, 좁혀도 소비 단정들은 조용히 초록이다.

        ⚠️ `_FN_ARGS`를 **모듈 전역에서 읽는다**(상수를 지역으로 베끼지 말 것) —
        베끼면 `_fn_body`가 쓰는 것과 갈려 이 단정이 공허해진다.
        """
        decls = re.findall(r"^function (\w+)\(", mock_src, re.M)
        assert len(decls) > 20, (
            f"목에서 최상위 `function` 선언을 {len(decls)}개밖에 못 찾았다 — 목이 "
            "선언 형태를 바꿨다면 이 계약을 갱신할 것(적게 잡히면 아래가 공허해진다)"
        )

        def findable(args_pat: str) -> set[str]:
            return {
                name
                for name in decls
                if re.search(
                    rf"function {re.escape(name)}{args_pat}\s*\{{(.*?)\n\}}",
                    mock_src,
                    re.S,
                )
            }

        wide = findable(_FN_ARGS)
        narrow = findable(_FN_ARGS_NARROW)
        assert narrow <= wide, (
            f"연 정규식이 좁은 것이 찾던 함수를 **잃었다** — 잃은 것: "
            f"{sorted(narrow - wide)}. 여는 것은 조이기여야 하고, 잃으면 완화다"
        )
        assert wide > narrow, (
            "연 정규식이 좁은 것과 **똑같은 집합**을 찾는다 — `_FN_ARGS`가 다시 "
            f"좁혀졌다(지금 모양: {_FN_ARGS!r}). 그 상태에서는 목이 규칙을 순수 "
            "함수로 여는 순간 이 파일이 함수를 못 찾아 빨개지고, 고쳐지는 것은 "
            "계약이 아니라 목 코드다 — 실제로 2026-08-20에 그렇게 되돌려졌다"
        )
