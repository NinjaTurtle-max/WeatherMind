...
⇒ 정규식을 `\\((?:[^)]*)\\)`로 열었다. **잡는 대상이 29 → 56으로 늘 뿐**(목의 최상위
`function` 선언 57개 기준, 2026-08-20 실측) 줄어드는 것이 없으므로 변이 보존 기준으로
**조이기**다. 함께 둔 것 둘: `_fn_body`의 **본문 도달 자기 단정**과
`TestParityInstrument` — **되돌림이 울게** 만드는 자리다(열기 전에는 되돌려도
아무것도 울지 않았다. 그 침묵이 바로 이 벽이 오래 산 이유다).
"""
...
def _fn_body(src: str, decl: str) -> str:
    ...
    **순수 함수로 여는 순간**(인자를 받는 순간) 이 계약은 함수를 **아예 못 찾아**
    ...
    잡는 대상이 **29개 → 56개**로 늘 뿐(2026-08-20 실측, 목의 최상위 `function`
    선언 57개 기준) 줄어드는 것이 없다.
