    ...
    **2026-08-08 갱신 — 그 일이 실제로 일어났다.** 시드가 237 → 272건이 되면서
    ...
    def test_0문항_칸이_실제로_다수다(self):
        ...
        **하한을 5로 잡은 산수**(2026-08-08 실측): 현재 공백 16칸 중 **10칸이
        ...
        ⚠️ **2026-08-19: 5 → 3으로 내린다.** 실측 공백이 **4칸**이 됐다
        (`city-anomaly-board × elementary·adult` · `risk-wildfire-board × adult` ·
        `bs-convection-board × adult`). 위 산수가 「저작이 표적하지 않는 6칸」을
        하한 근거로 삼았는데, **MT-18 대기역학 보드(expert)가 그중 하나를 채웠다** —
        그 저작은 밴드 공백을 표적한 것이 아니라 **개념 축을 넓히다 우연히 채운 것**이다.
        ⇒ 「표적하지 않는 칸 수」라는 하한 산정법 자체가 이번에 틀렸다: **표적하지 않아도
        채워진다.** 남은 4칸이 전부 **board 유닛 × 밴드**라 board 저작이 계속되면 또
        줄어든다. 3은 거기서 한 칸의 여유이고, **이 계약의 목적은 「공백이 많다」가 아니라
        「공백이 존재해 폴백 경로가 실제로 돈다」**이므로 하한이 낮아져도 값이 남는다.
        """
        holes = [key for key, n in cell_counts().items() if n == 0]
        assert len(holes) >= 3, f"칸이 다 찼다면 이 계약을 갱신할 것: {holes}"
