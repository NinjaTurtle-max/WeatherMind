...
def test_셔플_목록에도_같은_공백이_있었다():
    ...
    통과했다(「아니」는 「않」의 부분문자열이 아니다). 확장 비용을 확장 **전/후** 나란히
    쟀다: `hint_contradicts`가 True를 내는 본시드 문항이 **전 0건 · 후 0건**이다
    (2026-08-18 · 시드 1,018건 · [575] 수리 반영). 부수 피해 없이 공백만 닫힌다.
    """
    for hint in (
        "세 번째 선지는 옳지 않다.",
        "세 번째 선지는 정답이 될 수 없다.",
...
def test_선택지도_선지를_뜻하는_말이다():
    ...
    `knowledge_level=7` · 1-기준 [575])이 「명사 없는 서수」로 오분류돼 게이트·셔플
    양쪽에서 빠졌다. 그 문항은 **정답이 2번인데 해설이 2번을 틀렸다고 말했다** —
    ⑥ⓐ가 잡으려고 있는 바로 그 결함이다.

    ⚠️ **문장이 합성인 이유**: 그 문항은 **PR #109에서 이미 수리됐다**(서수 참조를
    없애고 선지 **내용**을 인용하는 형태로 갔다 — 이 게이트가 지시하는 처방 그대로다).
    그래서 시드에는 이 형태가 **한 건도 남아 있지 않고**(2026-08-18 실측: 해설에
    「선택지」 0건 · 시드 1,018건), 합성 문장이 유일한 고정 방법이다. 아래 문자열은
    **수리 전 [575]의 해설 꼬리**를 그대로 옮긴 것이다.

    ⚠️ **그래서 「선택지」 추가의 부수 피해는 0이다**(실측: 명사 있는 서수 7건 → 7건 ·
    새 적발 0건). 그러나 계약이 없으면 다음 사람이 「쓰이지도 않는 낱말」이라며 지운다 —
    **지워지면 같은 오분류가 그대로 돌아온다.** 이 픽스처가 그 낱말의 소유자다.
    """
