...
def test_명사_없는_자리_참조는_의도된_미탐이다():
    """🔴 **이것은 「놓친 것」이 아니라 「고르고 남긴 것」이다** (PM 승인 2026-08-14).

    「두 번째는 오독이다」는 **진짜 결함**인데 이 게이트를 통과한다. 명사 가드
    (「~ 선지/보기/답지」)를 요구하기로 했기 때문이다. 그 대가로 오탐 4종
    (빈도 「평균 3번 상륙」·단계 번호 「\u2460 단계에서」·횟수 「하루 각각 2번씩」·
    절 경계)이 사라졌다.

    **왜 이쪽이 맞나** — ⑥ⓐ에는 설계상 래칫이 없다(`test_래칫에_위치_모순_검사가_없다`가
    그 없음을 지킨다). 그래서 오탐은 **탈출구가 없다**: 맞는 문항이 CI를 붉게 만들고,
    사람은 「맞는 해설을 고쳐 쓰는 것」 말고 할 수 있는 게 없다. 반면 이 미탐은
    **잡는 시점이 뒤로 갈 뿐**이다 — `shuffle_answer_positions`는 계속 명사 가드
    없이 넓게 보므로(`hint_contradicts`) 위치 재배치 시점에 걸린다.

    ⚠️ **이 테스트가 있는 이유**: 다음 사람이 이 미탐을 「발견」해 창을 다시 넓히면
    오늘 잡은 오탐 4종이 그대로 되살아난다. 맞바꿈은 주석이 아니라 **테스트가
    소유해야** 다시 안 뒤집힌다. 넓히려면 이 테스트를 먼저 지워야 하고, 그때
    위 오탐 픽스처들이 대신 운다.
    """
    item = _mc("두 번째는 오독이다.", options=OPTIONS, answer="비가 온다")
    assert lint.hint_position_errors(item) == [], "의도된 미탐이 아니라 판정이 바뀌었다"

    # 그리고 **셔플 쪽은 이것을 본다** — 미탐이 영구 손실이 아니라는 증거다.
