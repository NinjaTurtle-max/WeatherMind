...
class TestSeedBoardRoundTrip:
    ...
    def test_시드_board_수_고정(self):
        ...
        # MT-18 전문가 보드 2판(2026-08-18): 52 → **54**. 태풍(order 54)·열대야(55)를
        # **말미에 붙였다** — 둘 다 난이도 3이라 난이도 3 구간 끝에 이어지고 뒤
        # 번호를 밀지 않는다(난이도 2였던 staging 3건은 중간에 끼워야 했다).
        # ⚠️ order 53은 비어 있다 — MT-19(#112)의 47 재배치가 채운다(그 브랜치 소유).
        assert len(self.boards) == 54
