...
> **[2026-08-09 추기 — 테스트 실명 정정]** 위 `test_R3_R5_시드_증보_47건`은 **현재
> 저장소에 없는 이름**이다. 이관은 실제로 처리됐고, 그 계약은 이름을 바꿔 가며
> 살아남았다 — 지금 총량을 고정하는 것은
> `backend/tests/test_seed_contract.py::TestSeedSchema::test_시드_증보_누적`이고
> 단정값은 **272건**이다. 본문은 R7 시점 기록이라 고치지 않는다. 이 문서를 근거로
> 테스트를 찾으려던 사람이 유령 이름에 걸리지 않도록 여기 표시만 남긴다
> (증보 때마다 이름에 건수를 넣던 관례가 원인이다 — 지금 이름에는 건수가 없다).
...
- backend 테스트 497건 통과, **1건 의도적 실패**:
  ...
  · **[2026-08-09 추기]** 이 이름도 현재 저장소에 없다(§5 추기와 같은 사유).
    후신은 `test_seed_contract.py::TestSeedSchema::test_시드_증보_누적`, 단정값 272.
