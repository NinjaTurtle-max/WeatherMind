...
앞서 Chaikin(모서리 깎기)을 썼다가 갈아탄 이유는 실측이다. 지금의 BASE_OUTLINE에서
**최소 곡률반경**(세 점 외접원 반지름의 최솟값)은
    원본 다각형 0.9 · Chaikin 4회 2.9 · 푸리에 K=4 13.3 · **K=5 9.4** · K=6 8.9
다(브라우저에서 실제 렌더된 곡선을 재면 9.0). 100폭 지도에서 반경 2.9는 눈에 각으로
읽힌다. 다각형의 "최대 방향변화(도)"는 샘플 간격에 딸려 움직여서 이 판단에 못 쓴다
— 반경으로 재야 한다. **윤곽을 고치면 이 수치도 다시 재서 갱신할 것**(아래
`__main__`이 원본·평활 두 값을 출력한다).
