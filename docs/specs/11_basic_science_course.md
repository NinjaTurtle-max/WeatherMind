...
기상 코스보다 가벼운 규모로 시작한다 — 선행 코스는 본 코스보다 가벼워야
이탈 없이 통과된다(선행 학습 앱의 기초 스킬이 짧은 것과 같은 원리. 벤치마킹 근거는
Observation_Report_02·03에만 둔다 — 대외 문서 언급 금지 규약).

> ⚠️ **2026-08-09 정정.** 이 문단은 "기상 코스(4섹션 12유닛)의 **2/3 규모**"라고
> 적고 있었다. 기상 코스는 그 뒤 「위험한 하늘」 4유닛이 붙어 **5섹션 16유닛**이
> 됐으므로(`database/seed/units.json` 실측) 비율은 2/3이 아니라 **1/2**이다.
> 비율을 다시 박지 않는 것은 어느 코스든 저작으로 계속 자라기 때문이다 —
> **유닛 수의 소유자는 `units.json`이고, 이 문서는 "더 가볍다"는 설계 의도만
> 소유한다.** 아래 트리 표(3섹션 8유닛)는 시드와 일치한다.
