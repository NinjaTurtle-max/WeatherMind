...
> **로드맵 위치**: 로드맵 마일스톤이 **아니다** — 마일스톤 2 완료 후 3 착수 전의
> **제품 완성도 트랙**(실사용 마찰 제거). 단 R10-B는 **마일스톤 3의 선행 인출**이다.
> 전략 SSOT: `docs/ROADMAP.md`.
...
### 2.1 관찰 보고서 항목 ↔ 스토리 대응 (연결 유지 — 누락 방지)

관찰 보고서 §3의 항목 A~I를 스토리로 옮긴 결과다. 이 표가 없어 **R10-I 누락을
문서만으로 판별할 수 없었다**(용어 규약: `docs/ROADMAP.md` §0).

| 항목 | 스토리 | 비고 |
|---|---|---|
| R10-A 에너지 정책 개편 (P0) | **S1** | 계약 §3.1 + D6·D8 |
| R10-B 콘텐츠 뱅크 확장+중복 방지 (P0) | **부분 → S1** | 중복 방지(단기)만 S1에 편입(D2). 뱅크 확장(중기)은 §5 범위 밖 — 마일스톤 3 선행 인출로 별도 스프린트 |
| R10-C WebGL 3D 단면 (P1) | **S2 · S3** | 단면 패널 / 지도 오버레이로 분할 |
| R10-D 온보딩 커밋 장치 (P1) | **S4** | 일일 목표 = D4 |
| R10-E 보드 풀이 보조 (P1) | **S5** | 언두·점진적 힌트 |
| R10-F 첫 화면 점진적 잠금 해제 (P1) | **S4** | D와 같은 파일 소유(App·탭바)라 병합 |
| R10-G 이탈 인텐트 + 콤보·칭찬 (P2) | **S5** | |
| R10-H 상호작용 마감 4건 (P2) | **S5** | |
| R10-I 시스템 보이스 (P2) | **미배정 — 판정 대기** | AI 캐스터를 온보딩·피드백·에너지·경로 안내로 확장. 의도적 제외인지 누락인지 문서로 판별 불가 → 클라이언트 확인 필요. 포함 시 S5 또는 별도 S7 |
| (보류) 예보 대결 PvP | — | 로드맵 6 이월 |
| **R10-J 투자 후 계정 유도 (P1)** | **미배정 — 신규(2026-08-01)** | 보고서 대조에서 발견된 누락. 계정 없이 체험 → 진도 저장 시점에 가입. 게스트 로그인 기반은 이미 있고 **온보딩 순서 재배치**가 실체. S4(온보딩·App·탭바) 소유와 겹침 → S4 편입 후보 |
| **R10-K 스캐폴딩 재적용 (P2)** | **미배정 — 신규** | 새 요소 최초 등장 시 가이드 재노출. S5(보드) 소유와 겹침 |
| **R10-L 자원 경제 (P2)** | **미배정 — 신규** | 프리즈를 소비재로 확장. 백엔드 자원 축 신설이라 S1과 별개 |
| **R10-M 기대치 하향 보이스 (P2)** | **미배정 — 신규** | R10-I의 하위 사례. I와 함께 판정 |
| **R10-N 잠금 화면 = 동기부여 화면 (P1)** | **S4 편입 — FE-3 통보 완료** | 로그인 상태 관찰(§3.2-2)로 **R10-F 계약을 대체**한다. 자물쇠 차단 폐기 → 설명+가치+진행형 조건+해제 CTA. 구현 중 통보했으므로 재작업 최소 |
| **R10-O 해제 조건 카운트다운 (P2)** | **S4 편입** | 고정 문구가 아니라 남은 횟수 계산 렌더. N과 같은 컴포넌트라 함께 |
| **R10-P 프로필 절제 + 빈 슬롯 노출 (P2)** | **미배정** | 통계 소수화 + 미획득 항목도 슬롯으로 목표화. S4의 프로필 접힘과 같은 파일 소유 → S4 편입 후보 |
| **R10-Q 퀘스트 남은 시간 (P2)** | **미배정** | 당일 마감 카운트다운. 백엔드 퀘스트 만료 시각 노출 필요 여부 확인 후 배정 |
...
- **점진적 잠금 해제**(프론트 게이트, 서버 권한 아님 — 표시 계층):
  ...
  | 리그 탭 | 세션 3회 완료 |
  해제 시 1회성 축하 토스트.
- **⚠️ 잠금 화면 계약 변경(2026-08-01, 관찰 보고서 §3.2-2 근거 — R10-N)**:
  기존 안 *"잠금 탭은 자물쇠 + 세션 N회 완료하면 열려요"*(**접근 차단**)를 **폐기**한다.
  벤치마킹 대상 실물(관찰 보고서 №3)은 정반대다 — **탭은 열고**, 그 자리에 ① 기능 설명("리더보드란
  무엇인가요?") ② 작동 방식("레슨 완료, XP 획득, 순위 경쟁") ③ 가치 제안
  ④ **진행형 해제 조건**("레슨을 **2개** 이상" — 진척에 따라 남은 횟수가 줄어든다)
  ⑤ **해제로 가는 행동 CTA**("레슨 시작하기")를 놓는다.
  → 잠금을 마찰이 아니라 **동기 부여 화면**으로 만든다. 자물쇠로 막지 마라.
  → 조건 문구는 고정값이 아니라 **남은 횟수 계산**으로 렌더한다(R10-O).
...
- **⚠️ 힌트 3단 추가 — 바로 위 「정답 배치를 직접 보여주지 않는다」의 범위가 바뀌었다
  (2026-08-14, N-3 ①안 · 클라이언트 승인)**: 위 2단 기술은 R10 시점에 참이었으므로
  **지우지 않고 그대로 둔다** — 왜 2단이었는지가 이 변경의 전제다. 그 위에 3단을
  얹는다: **3회 미통과 후 `board_rules.json`의 `explain` 전문을 연다.**
  1·2단 사다리는 손대지 않는다.
  → **3단은 정답 요소명을 공개한다.** 실측 근거: subtype 조건을 가진 규칙
    **10건 중 8건**이 `explain`에 요소명을 그대로 적는다
    (`cold_front_shower` → "한랭전선은…", `okhotsk_sea_fog` → "오호츠크해 기단은…").
  → **여전히 미공개인 것**: 배치 좌표 · 수치 임계 · 규칙 조건 원문 ·
    팔레트 하이라이트 · 문항 저작 `hints`. 3단이 여는 것은 `explain` 하나뿐이다.
  → **왜 계약을 여는가**: board 문항 전건이 `template_json.correct_answer`가 빈
    값이고(규칙 엔진이 채점 권위라 그것이 정상이다) 힌트가 2단 상한이라,
    **못 푸는 학습자가 답에 닿는 경로가 하나도 없었다**(N-3). N-3이 요구한 것이
    「답에 닿는 경로」 자체이므로 3회나 틀린 학습자에게 답을 가리는 것은 목적에
    반한다. 게다가 `board_rules.json`은 이미 `GET /board/rules`로 **전량**
    클라이언트에 내려간다 — 3단은 새 유출면이 아니라 **표시 게이트**다.
  → ⚠️ **승인 근거가 한 번 틀렸다는 사실을 남긴다.** 최초 보고는
    *"explain은 현상 해설이라 계약을 깨지 않는다"*였고 **거짓이었다**(위 10건 중 8건 실측).
    클라이언트는 그 실측을 받은 뒤 ①안 유지를 재확인했다 — 정답 요소가 열린다는
    것을 **알고 고른 것**이다. **그 문장을 근거로 재인용하지 말 것.**
  → 소유자: N값은 상수 `EXPLAIN_AFTER_MISSES`(`frontend/src/modules/board/
    AtmosphereBoard.jsx`, 근거 주석 동봉) 하나뿐이고, 드리프트는
    `boardAssistRetention.smoke.test.mjs` 「①안: N회 미통과 후 현상 해설이
    열린다(그 전에는 안 열린다)」가 문다. 적용 범위는 **연습 탭뿐** — 세션 경로는
    재제출이 없어 미통과가 쌓이지 않는다.
