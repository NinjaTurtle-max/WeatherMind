...
### D8. 계약 테스트 작성 중 드러난 세부 판정 (SA-1 질의 → PM 확정)
1. **§3.1 "차단 지점 2곳"은 오기** — 나열은 3개다(§3.1:68-70). D6의 3곳이 정답.
2. **보드 상세 응답 스키마**: 새로 만들지 않고 기존 `BoardPuzzle` 단건 재사용
   (`content_item_id`·`template_json`·`cleared`·`difficulty` — 목록 원소와 동일).
3. **`consume_if_available`의 0행 분기 반환값**: `0` 하드코딩 금지. 가드가
   `clouds >= CLOUD_COST`이므로 COST가 env로 2 이상이 되면 0행에서도 잔량이 0이
   아닐 수 있다 → **해당 행을 재조회해 실측값 반환**(+ `user.clouds` 동기화).
4. **`plan_consume` 고아화 대응**: 삭제하지 않고(§3.1 시그니처 불변) **오라클로
   승격** — 순수 모델과 DB 경로(`require_entry`)가 경계(0·19분·20분·MAX)에서
   일치함을 계약 테스트로 묶는다. `require_entry`는 `plan_consume`을 호출하지
   않고 `apply_regen`+명시 검사로 구현한다(감소값을 버리는 호출은 의도 불명).
5. **유닛 세션 풀도 D2 범위에 포함 — 단 best-effort**. 유닛 세션에는 생성 폴백이
   **없다**(`curriculum_service.py:415` "문항 풀이 비면 0문항 세션이 발급된다").
   하드 제외하면 같은 유닛 당일 재진입 시 **0문항 세션**이 발급돼 반복보다 나쁜
   회귀가 된다 → `_unit_content_pool`은 오늘 응답분을 제외하되 결과가
   `UNIT_SESSION_SIZE`보다 적으면 **제외분으로 백필**한다(신선도 우선, 굶기지 않음).
6. **배치고사 일괄 제출 경로에는 에너지 호출이 애초에 없다**(`submit_answers_bulk`
   — `test_placement_bulk.py:143-148`이 이미 고정). 계약 4는 순수 판정으로만 검증.
...
- **`preview` vs `zoneVisuals` 이중 진실원**: `PeninsulaMap.jsx`가 오버레이(구름·
  ...
  미리보기 `preview`(`:167-168`)를 쓴다. 서버 판정이 로컬과 다르면 같은 존에서
  오버레이와 아이콘이 어긋난다. 추출 전부터 동일한 동작.
  **웨이브 1에서 고치지 않는다** — 순수 리팩터·WebGL 변경과 섞이면 리뷰에서
...
- **mock의 하루 경계가 서버와 9시간 어긋난다**(기존 성질, 웨이브 2 스모크 대상):
  `apiMockPlugin.js:44`의 `todayISO()`는 `toISOString()` = **UTC** 기준이고, 서버의
  "오늘"은 KST(`datetime.now(KST).date()`)다. 즉 목의 하루는 **09:00 KST**에 넘어간다.
  R2부터의 목 전역 규약이라 단독 수정 시 `session_date`·`quiz_id` 채번까지 파급되므로
  웨이브 0에서 손대지 않았다. 새 `today_answered_count`가 이 경계를 쓰게 되면서
  FE-3이 "자정 리셋"을 기대할 때 목에서는 오전 9시에 리셋되는 것으로 보인다
  (**실서버는 정상 — 목만 시프트**). 웨이브 2 통합 스모크에서 확인한다.
...
## 4.2 웨이브 0 종료 보고 (2026-08-01)

**편성**: SA-1 1인 + FE-2 1인(마운트 경계 추출) + PM 결정. §2.6 기준 증원 대상
아니었음 — 계약 테스트는 파일 소유를 쪼갤 여지가 없고, 추출은 단일 파일 리팩터.

**산출물** (브랜치 `chore/r10-00-wave0-contracts`, 커밋 11건)
- 계약 테스트 2파일 100건 — **78 빨강**(구현 대기, 계약 1~20) / 22 초록(회귀·수치 고정).
  기존 761건 회귀 0(전체 783 = 761 + 22), collection 에러 0.
- `AtmosphereBoard.jsx` 788 → 521줄 분해 · `PeninsulaMap.jsx` 249줄 ·
  `boardLayout.js` 36줄(동결 계약) + 좌표 계약 테스트 25 assert(변이 검증 완료).
- mock 계약 선반영 — 프론트 3인이 백엔드 구현 없이 착수 가능.
- 결정 기록 D1~D10.

**게이트**: `/code-review` 실행 → P1 1건·P2 2건 지적 → 전건 반영 후 재검토 완료
(`a438bb6`). 리팩터 동작 무변경은 PM이 본문 바이트 동일성으로 독립 검증.

**웨이브 1 착수 조건 충족**: 계약 고정 ✅ · 파일 소유 분리 ✅ · mock 선반영 ✅ ·
목표 저장 방식 확정 ✅(D4) · 마운트 인터페이스 확정 ✅(D3).

**남은 PM 판단** — 웨이브 1 편성. SA-1 부하가 초기 계획을 초과했다(S1 에너지 +
뱅크 중복 3경로 + `0008`·일일목표 API + `clouds_spent` 스키마). 파일이 겹치지
않아(에너지 ↔ 풀 ↔ 진척) §2.6대로 SA-2 증원이 가능하나 그러면 동시 6으로 상한
초과 → (a) SA-1 원자 커밋 3개 순차 또는 (b) SA-2 증원 + FE-4(대부분 P2)를
웨이브 2로 이월. **권고 (b)** — P0 2건이 1인 순차에 묶이는 것이 최대 리스크.

**R10-B 재평가 필요**: D2로 "첫날 중복"의 근거가 뱅크 규모에서 **쿼리 누락**으로
바뀌었다. 뱅크 확장 자체의 가치는 남지만 P0 근거는 사라졌으므로 우선순위를
재평가한다(§5).
