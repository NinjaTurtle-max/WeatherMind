...
### 웨이브 1 코드 리뷰 (2026-07-21, PM 타겟 리뷰)

각 직군 자체 테스트가 탄탄해(백엔드 pytest 326·AI 39·프론트 build·데이터 해존재 검증)
PM 타겟 리뷰로 진행. 핵심 리스크(보상 루프 XP 정확성)는 백엔드 테스트로 커버 확인:
퀘스트 재계산 멱등·완료 1회 지급, 티어 경계(1099/1100/1549/1550), 대결 예측 결정성·
노이즈 범위·승패 정산, 배지 멱등, backend↔celery 티어 파리티.

**확정 결함 2건 (PM 직접 교차 직군 수정)**
1. AI↔QA 시그니처 불일치 — AI가 run_heuristic_checks(question, concept_tag)로 분리
   (§3.6 based_on의 anomaly 판정용)했으나 QA test_seed_contract가 concept_tag 없이 호출
   → 재현 퍼즐 2건 게이트 실패. QA 테스트가 item["concept_tag"] 전달하도록 정합
   (수정 후 seed_contract 103건·백엔드 326건 통과 검증).
2. celery beat에 settle_daily_duel 크론 미등록 — 백엔드가 태스크는 정의했으나 소유권상
   celery_app.py 미수정. PM이 매일 04:00 크론 등록(리그 정산 03:30 이후).

**회고**
- 잘된 것: 대결 AI 예측을 LLM 없이 결정적 노이즈로 설계 → 키 부재·테스트 환경에서도
  완전 재현 가능. 퀘스트를 이벤트 카운터가 아닌 당일 집계 재계산으로 만들어 멱등성 확보.
  구름 티어 네이밍으로 독자 세계관 유지(§3 벤치마킹 원칙).
- 아쉬운 것: 소유권 경계(celery_app은 DevOps, test는 QA)가 교차 직군 결함을 낳음 —
  R5는 공유 시그니처 변경 시 소비자 목록을 계약에 명시. daily_xp 퀘스트가 약점 배율을
  제외한 기본 XP 집계라 UI 표기와 미세 괴리 가능(문서화된 결정, R5 검토).
- 범위 밖(로드맵 유지): 탐정 스토리·AI 캐스터 롤플레이(LLM 키), 웹푸시, IRT.
