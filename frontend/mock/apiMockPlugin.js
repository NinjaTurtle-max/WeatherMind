...
//       서버가 권위라 그대로 베낀다.
//    🔴 **정정(2026-08-20 실측): 「계약이 볼 수 없다」가 거짓이었다.**
//       종전에 *"JSON을 건너면 파이썬도 int 3으로 읽으므로 계약도 이 갈래는 볼 수
//       없다"*고 적혀 있었다. **재 보니 아니다:**
//         파이썬  json.loads('{"board_order": 3.0}') → 3.0 · isinstance(int) False → 10000
//         JS      JSON.parse(...)                     → 3   · Number.isInteger true → 3
//       **갈리고, JSON을 건너서도 보인다.** 「볼 수 없는 갈래」가 아니라
//       **표본에 안 넣은 갈래**였다 — 오늘 다섯 번 잡은 그 형태다.
//    ⚠️ 다만 **목이 서버를 따라갈 수는 없다.** `JSON.parse`가 `3.0`을 `3`으로
//       접으므로 JS 쪽에서 구분을 되살릴 방법이 없다. 그래서 이것은
//       **「측정 불가」가 아니라 「재현 불가한 실제 불일치」**다.
//    ⚠️ 지금은 **잠재**다 — 시드 원문에 소수 `board_order`가 **0건**임을 실측했다
//       (`grep '"board_order"\s*:\s*-?\d+\.\d+'` → 없음). 저작이 하나라도
//       소수를 쓰면 그 칸의 순서가 목과 서버에서 갈린다.
//    🔴 **재정정(2026-08-21 실측): 편입을 미룬 사유가 틀렸다.**
//       `1ddae33` 본문이 ~~"넣으면 파리티가 붉어지므로 면제 사유를 달아야 하고,
//       그건 계약을 무르는 쪽"~~이라고 적었다. **붉어지지 않는다:**
//         `JSON.stringify({board_order: 3.0})` → `'{"board_order":3}'` → py `int 3` → 키 3
//       객체 표본에 넣으면 평범한 정수와 **바이트가 같아져 조용히 초록**이다.
//       미뤘어야 할 진짜 이유는 「붉어서」가 아니라 **「울지도 않는 갈래에 면제를
//       달게 되어서」**다 — 오늘 열세 자리에서 걷어낸 바로 그 모양이다.
//    ⚠️ 두 서술이 어긋나 보인 것은 **출발점을 안 적었기 때문**이다. 답은 출발점이
//       가른다:
//         객체에서 출발  → JS가 직렬화하며 접는다 → 갈림이 **죽는다**
//         원문 텍스트에서 → py만 `3.0`(float)으로 읽는다 → 갈림이 **산다**
//       시드 경로는 **후자**다(파일 텍스트를 `JSON.parse`한다). 그래서 갈림은
//       실재하고, 아래 표본을 값이 아니라 **텍스트**로 싣는 이유가 이것이다.
