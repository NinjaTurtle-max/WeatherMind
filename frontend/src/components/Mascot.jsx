 ...
 *               ⚠️ **그림이 2026-08-18에 교체됐다**(사용자 지시) — 배정·이름은
 *               그대로고 PNG만 바뀌었다. 파일명이 같아 diff에 'Bin' 한 줄로만
 *               남으므로 여기 적어 둔다.
 *   raincloud   비구름 개념 전용(홍수 대응). 2026-08-10 합류
 *   fire        불     개념 전용(산불 기상). 2026-08-10 합류
 *   thermometer 온도계 개념 전용(온도와 열). 2026-08-10 합류
 *   wind        바람이 개념 전용(압력의 기초). 2026-08-18 합류
 *   grass       풀     개념 전용(복사와 에너지 수지). 2026-08-18 합류
 *
 * 뒤의 여덟은 **화면이 아니라 개념에 붙는다**. 기초과학 코스가 들어오면서 개념
 ...
 * **2026-08-18에 둘이 더 합류해 14장 — 개념 14종과 완전 1:1이 됐다.** 마지막까지
 * 남아 있던 겹침 둘(구름이 = 기압과 전선·압력의 기초, 태양이 = 열섬·복사와 에너지
 * 수지)이 바람이·풀로 풀렸다. `conceptCharacter.js`가 *"그림이 둘 더 생기면 여기만
 * 고친다"*고 적어 둔 그 자리다 — 이제 폴백(구름)으로 떨어지는 개념 태그는 없다.
 *
 ...
 * 따라 404가 난다.
 * ⚠️ **한글 이름 원본 3장이 실제로 들어와 있었다**(`눈구름/바람이/풀.png`,
 * GitHub 웹 업로드 `288441b`). 어디서도 참조되지 않는데 `dist/`에는 실려 나가고,
 * SRC를 통하지 않으니 ① 여백 검사도 비켜 갔다 — 2026-08-18에 걷었다. 새 그림은
 * **SRC에 등록되는 ASCII 이름 한 벌만** 저장소에 남긴다.
 *
