...
export default {
  board: {
    ...
    page: {
      ...
      // 🔴 **2026-08-20 판정 3 — 이 두 줄을 다시 썼다.** 종전 값과 사유를 남긴다:
      //   · 종전 제목: '🔒 지금 학습 수준에서 열리는 난이도까지 보여요'
      //   · 종전 본문: '초등학생은 초등, 중·고등학생은 중·고등까지, 성인은 전부 열려요.'
      //   · 사유 ⑴ **「성인은 전부」가 거짓이 됐다.** 잠금 축이 학령 파생 난이도(3칸)에서
      //     지식 단계(10칸)로 갈아탔고, 성인 위에 밴드가 하나 더 있어 성인도 위 단계는
      //     못 연다(실측 판수는 저장소 문서가 소유한다 — 여기 적지 않는 이유는 아래 ⑶).
      //   · 사유 ⑵ **학령 기반 서술 자체가 죽었다** — 밴드는 천장의 소유자가 아니다
      //     (판정 1). 천장의 새 소유자는 **판정 대기**(추천 판정 ↔ θ 파생)라, 문구는
      //     소유자가 누구든 참인 「내 단계까지」로만 말한다.
      //   · 사유 ⑶ 🔴 **단계 수도 판수도 숫자로 못박지 않는다.** 그 숫자가 갈리면 또
      //     거짓이 된다 — 같은 날 「하루 9문항」이 정확히 그 형태로 틀렸다.
      //   · 배지 낱말과의 「한 몸 계약」은 2026-08-20에 버렸다(축이 갈라졌다) —
      //     경위는 boardEntryGate 스모크 #32b가 소유한다. 대신 배지와 **같은 축의
      //     낱말**(「단계」)을 쓴다.
      lockedBannerTitle: '🔒 지금은 내 단계까지 열려요',
      lockedBannerBody: '내 단계까지의 퍼즐이 열려 있어요. 위 단계는 단계가 오르면 함께 열려요.',
      // 🔴 **종전 「학습 수준 바꾸기」는 두 겹으로 거짓이었다**(2026-08-21 정정).
      //   ⑴ 이 CTA가 보내던 `/me`에는 **학습 수준 카드가 없다** — 클라이언트 판정
      //      「진입에서 한 번 고르면 고정이야」로 걷혔다(`91e42de`).
      //   ⑵ 있었다 해도 **신고할 수 있는 밴드는 3종뿐**이라(`schemas/auth.LevelGroup`)
      //      재신고로 닿는 천장은 `adult` 사전값의 파생이 상한이다. **상위 4층
      //      (전문가 보드)은 신고로 영영 안 열린다.**
      // ⇒ 참인 행동으로 바꾼다: **풀면 θ가 오르고 단계가 오른다.** 그것이 상위
      //   층을 여는 유일한 길이고, 「측정으로 연다」는 이 제품의 축 그대로다.
      // ⚠️ 「나중에」·「언제든」·「바꿀 수 있」을 쓰지 않는다(uiCopy.contract ⑹⑺ —
      //   못 지킬 약속의 어법). 「풀면 오른다」는 지금 참이다.
      lockedBannerCta: '풀면서 단계 올리기',
      blockedSuffix: ' (구름 부족)',
  ...
  explore: {
    ...
    typhoon: {
      ...
      // 배너 오른쪽 한 줄(2026-08-19 사용자 지시로 문구까지 지정됐다).
      // ⚠️ **짧게 유지할 것** — `tightDescription`(10px)로 한 줄에 드는 길이가
      // xl에서 430px이다. 낱말을 늘리면 두 줄로 접히며 배너가 더 높아진다.
      heroDesc: '해수면 온도, 연직 시어에 따라 태풍이 언제, 어떻게 발전하는지 알아보세요',
      ...
      // ── 2막 구성(2026-08-21 사용자 지시) ─────────────────────────────────
      // "모식도 두 개가 태풍의 개념 설명이잖아 이걸 먼저 크게 보고 다음으로
      //  미션을 깨러 가는 느낌으로". 화면이 「알아보기 → 만들어보기」 두 막이 된다.
      stage1Label: '1단계 · 알아보기',
      stage1Title: '태풍은 어떻게 생겼을까요?',
      stage1Body: '두 그림을 먼저 보고 나면, 다음 화면에서 직접 바다를 데워 태풍을 만들어 볼 거예요.',
      stage2Label: '2단계 · 만들어보기',
      toMission: '태풍 만들러 가기 →',
      skipToMission: '바로 만들러 가기',
      backToConcept: '← 개념 다시 보기',
      cta: '🌪️ 태풍 개념 문항 풀기 — 학습 경로에서 이어가기',
    },
    climate: {
      ...
      cta: '🌡️ CO₂와 기후 개념 문항 풀기 — 학습 경로에서 이어가기',
    },
  },
};
