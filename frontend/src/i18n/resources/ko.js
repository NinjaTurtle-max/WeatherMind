...
export default {
  ...
  curriculum: {
    // 튜터 진도 한마디 (2026-08-09) — 진입 카드의 남는 세로를 채운다.
    // **수치는 전부 실데이터**(트리의 섹션별 유닛 수·클리어 수)다. "거의 다 왔어요"
    // 같은 어림 문구를 쓰지 않는 이유: 몇 칸 남았는지 모르면 격려가 공허해진다.
    // 하루 목표 미설정 — 진입 카드에서 내 정보(설정 통로)로 보낸다
    goalUnset: '목표를 설정하세요!',
    tutorProgress: {
      first: '「{section}」 첫 칸이에요. 여기서 시작해요!',
      mid: '「{section}」 {total}칸 중 {cleared}칸을 마쳤어요.',
      last: '「{section}」 마지막 칸이에요. 하나만 더!',
      daily: '오늘 몫부터 가볍게 풀어 볼까요?',
      done: '오늘 할 학습을 다 마쳤어요. 훌륭해요!',
    },
