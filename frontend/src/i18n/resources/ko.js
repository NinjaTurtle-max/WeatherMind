...
export default {
  ...
  saveProgress: {
    ...
    nodeCta: '정보 입력하러 가기 →',
    cardTitle: '진도 저장',
    // ⚠️ **「다른 기기에서도 이어서 배울 수 있어요」를 여기 되살리지 말 것**
    //    (2026-08-13 실서버 검수). 세 문구가 전부 그렇게 말했는데 **돌아올 문이
    //    없다** — `App.jsx`에 `/login` 라우트가 없고(`a4a8b6f`가 화면과 함께 걷었다)
    //    `:327`이 모르는 경로를 전부 `/`로 튕긴다. 다른 기기로 열면 그냥 **새
    //    게스트**가 되고, 이 브라우저의 localStorage를 지워도 같다.
    //    그래서 지금 참인 것만 적는다: **저장한 정보가 계정에 붙는다**는 것.
    //    「언제든 복구」도 쓰지 않는다 — 그것 역시 오늘 거짓이다.
    // ⚠️ 되살리려면 `/login`(ⓑ) 복구가 **먼저**다. 순서를 바꾸면 못 지킬 약속이
    //    다시 실서버에 뜬다. `tests/onboardingSave.contract.test.mjs`가 그 순서를
    //    조건부로 문다(라우트가 없는 동안에만 이 문구를 금지한다).
    cardBody:
  ...
  auth: {
    ...
    convert: {
      ...
      bodyStrong: '그대로',
      bodySeg2: ' 내 계정이 돼요.',
      // ⚠️ 종전 '어느 기기에서든 이어서 학습할 수 있어요.' — **거짓이었다**(2026-08-13
      //    실서버 검수). `/login` 라우트가 없어 다른 기기로 열면 새 게스트가 된다.
      //    경위와 되살리는 순서는 `saveProgress` 블록 주석이 소유한다.
