...
export default {
  ...
  common: {
    ...
    /**
     * 온도 단위 기호 — **언어마다 다른 것이 정상이다**(92e146e 판정).
     * ko = `℃`(U+2103, KS X 1001 계보의 한국어 조판 관례).
     * en은 U+00B0 뒤에 C를 붙인 **두 글자**다(en.js 짝 주석 참조).
     * 통일하려 들지 말 것: U+2103은 CJK 호환 문자라 영문 본문에서 CJK 폰트로
     * 폴백해 글자폭이 튀고, NFKC도 그 두 글자로 분해한다.
     * **기호 하나짜리 키인 것도 의도다** — `{v}℃` 같은 템플릿으로 만들면
     * 소비처 12곳마다 공백·순서가 갈리고, 그 흔들림이 바로 이 키가 없앤 것이다.
     * 컴포넌트 하드코딩 회귀는 uiCopy.contract가 양방향(ko 리터럴·en 리터럴)으로 문다.
     *
     * ⚠️ 이 주석에 en 표기를 **글자 그대로 적지 않은 이유**: uiCopy.contract의
     *    「컴포넌트에 하드코딩된 en 표기가 없다」 검사는 주석을 걷지 않고 원문을
     *    훑는다(en 리소스 두 계열만 제외). 적으면 그 계약이 거짓으로 운다.
     */
    celsius: '℃',
  },
  nav: {
  ...
  errorBoundary: {
    ...
    body: '예상하지 못한 일이 생겨 화면을 그리지 못했어요. 다시 시도해 주세요.',
    retry: '다시 시도',
    home: '학습 화면으로',
  },
  ...
  entryInfo: {
    ...
    submit: '다음 — 실력 진단 받기 →',
    skip: '건너뛰기 →',
    // 🔴 **거짓 약속이었다**(2026-08-20 클라이언트 판정 *「진입에서 한 번 고르면
    //   고정이야」*). `/me`의 학습 수준 카드는 #148이 걷었고 되살리지 않기로
    //   확정됐다 — 즉 **「내 정보에서 바꿀 수 있는」 통로가 0곳**인데 첫 화면이
    //   그렇게 약속하고 있었다. 되돌아가지 않게 `uiCopy.contract`가 이 줄을 문다.
    // ⚠️ 바로 위 `goalHint`의 「나중에 바꿀 수 있어요」는 **다른 축**(하루 목표)이고
    //   아직 판정 대기라 손대지 않았다.
    note: '지금 고른 수준으로 바로 시작해요.',
    ...
    // 🔴 **재방문 복귀 화면**(2026-08-19 지시 ⑫-b). 첫 방문의 대문(위 `title`·`body`)과
    //    **짝**이라 같은 블록에 둔다 — 같은 `/` 자리의 두 얼굴이다.
    // ⚠️ **아무것도 다시 묻지 않는다.** 수준·닉네임·목표를 재방문에 다시 물으면
    //    ④⑤ 반려가 재발한다(내 정보에서 걷어낸 그 건). 그래서 이 블록에는 라벨·
    //    플레이스홀더가 **없다** — 문구 넷이 전부이고, 그 사실 자체가 계약이다.
    //    여기에 입력 문구를 더하려는 사람은 먼저 그 반려부터 읽을 것.
    // ⚠️ 「로그인」·「회원가입」 금칙어는 여기도 그대로다(규정).
    return: {
      title: '다시 오셨네요!',
      body: '어제까지 배운 자리에서 이어서 하면 돼요. 다시 정할 건 없어요.',
      continue: '이어서 학습 →',
      // 부 CTA — 다른 기기에 저장해 둔 진도를 여는 사람용이고 **선택**이다.
      // 주 동선(SideNav·TabBar·헤더)에는 여전히 0건이다(MT-29 계약).
      load: '진도 불러오기',
    },
  },
  placement: {
    // ⚠️ 이 `skip`은 **진단을 통째로 버리고 홈으로 나가는** 헤더 버튼이다
    // (PlacementPage). 문항 하나를 넘기는 「모르겠어요」와 **다른 버튼**이므로
    // 재사용하지 말 것 — 같은 문구면 학습자가 둘을 구별할 수 없다.
    skip: '건너뛰기 →',
    // 「모르겠어요」(2026-08-19 클라이언트 지시) — 문항 스킵. 낱말 「건너뛰기」를
    // 일부러 피한다(위 skip과의 혼동 차단). 오답 처리라는 사실을 **누르기 전에**
    // 말한다: 스킵은 미응답이 아니라 오답이고, 그것이 서버 채점 계약이다.
    dontKnow: '모르겠어요 — 오답으로 넘어가기',
    dontKnowNote: '답을 고르지 않고 넘어갈 수 있어요. 모르겠다고 한 문항은 오답으로 채점돼요.',
    title: '실력 진단 — 내 수준 찾기',
