...
export default {
  ...
  // ⚠️ **2026-08-19: 이메일·비밀번호를 걷고 닉네임 하나로 바꿨다.** 경위를 남긴다 —
  //    종전 `body`가 *"저장할 때 쓴 이메일과 비밀번호를 넣으면"*이라고 안내했는데,
  //    진입 화면이 물어보는 것은 **닉네임뿐**이었고 게스트의 비밀번호는 무작위
  //    시크릿이라 **원리적으로 아무도 그 문을 못 열었다.** 이름을 적게 해 놓고 그
  //    이름으로 못 돌아오는 상태가 실서버까지 갔다. 되돌리려면 서버
  //    `POST /auth/resume`부터 되돌려야 한다.
  loadProgress: {
    fromSave: '이미 저장하셨나요? 진도 불러오기',
    title: '진도 불러오기',
    body: '진도를 저장할 때 정한 닉네임을 적으면 그때 진도로 돌아가요.',
    nicknameLabel: '닉네임',
    nicknamePlaceholder: '예: 구름사냥꾼',
    submit: '진도 불러오기',
    submitting: '진도 불러오는 중…',
    // 실패는 **세 갈래**다 — 서버가 왜 못 찾았는지 말해 주지 않으면 학습자는
    // 자기 이름을 의심할지 오타를 의심할지 모른다(서버 코드와 1:1).
