...
export function createBotRenderer(canvas, mesh) {
  ...
  // 배경 투명. 한때 이 자리에 "버튼의 흰 원이 그대로 비친다"고 적혀 있었으나 그
  // 흰 원은 걷어냈다(2026-08-13 클라이언트 지시 — 캐릭터가 버튼처럼 보였다).
  // 투명이어야 하는 이유는 그때보다 지금 더 강하다: 뒤에 아무 판도 없으므로
  // 불투명하게 지우면 캐릭터 주위에 검은 사각형이 그대로 남는다.
