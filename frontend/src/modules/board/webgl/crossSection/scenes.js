...
const floodRiskSaturatedInflow = () => [
  ...
  //      덮어 투수/불투수 대비를 부쉈다.
  // 🔴 **8차(2026-08-19): 클라이언트가 침수 도판을 참고로 지정**(freepik
  //   "FLOOD ISOMETRIC"). ⚠️ 따라 그리지 않았고 **규약 셋만** 가져왔다:
  //     ⓐ **물이 진한 청록이고 두껍다** — 종전 `#38bdf8` alpha 0.50은 연한
  //        하늘색이라 「젖은 지면」으로 읽힐 수 있었다. `#0891b2` alpha 0.58로
  //        옮긴다. 잠긴 벽은 42%가 비쳐 보인다(참조도 벽이 물 속에 비친다).
  //     ⓒ **물층이 단면 측면에서 두께로 보인다** — 참조는 블록 옆면에 청록 층이
  //        두껍고 그 아래 갈색 흙층이 있다. `y0` -0.006 → **-0.030**.
  //        ⚠️ **수면(y1)은 안 올렸다** — 실척은 수면 높이가 정하므로 그대로다.
  //        지표 아래로 물이 차 있는 것은 포화(§3C)의 표현이라 사실에도 맞다.
  vol({ x0: 0.335, x1: 0.98, y0: -0.030, y1: H(0.105), z0: 0.14, z1: Z,
    color: rgba('#0891b2', 0.58), pattern: 3, at: 3 }),
