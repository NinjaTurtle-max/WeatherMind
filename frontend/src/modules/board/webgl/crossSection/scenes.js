...
/**
 * 🔴 **차를 빼고 네 채로, 간격을 벌린다**(2026-08-20 클라이언트:
 * *"자동차 없애고 건물을 추가하는 데 간격을 조금 벌려"*).
 *
 * ⚠️ **깊이의 자가 차에서 건물로 넘어온다.** 조사 §2⑵는 *"전 세계 공통으로 차를
 *    쓴다"*고 적었고 2차 반려의 사유가 「잴 물건이 없어서」였다 — 그 자리를 비워
 *    두면 같은 반려로 돌아간다. 대신 조사 §2⑺의 다른 관례를 세운다:
 *    ***잠긴 정도의 4단계.*** 한 채가 얼마나 잠겼는지가 아니라 **네 채가 저마다
 *    다르게 잠긴 것**이 깊이를 말한다.
 *    수면 0.042에 h를 대면 **33% · 46% · 62% · 76%** — 76%가 조사가 말한
 *    「지붕만 남은 집」이고, 그 옆에 33%인 「벽 절반인 집」이 서 있다.
 * ⚠️ 간격은 중심 0.13 → **0.15**(가장자리 틈 0.078 → 0.100). 동쪽 끝이 0.865로
 *    밀리므로 **유출 화살표를 그만큼 짧게** 잡아야 실루엣에 안 얹힌다(아래 참조).
 */
const FLOOD_CITY = [
  { x: 0.39, h: 0.092, w: 0.050, color: rgba('#a8a29e', 0.95) },
  { x: 0.54, h: 0.126, w: 0.054, color: rgba('#94a3b8', 0.95) },
  { x: 0.69, h: 0.055, w: 0.048, color: rgba('#a8a29e', 0.95) },
  { x: 0.84, h: 0.068, w: 0.050, color: rgba('#94a3b8', 0.95) },
];
...
const floodRiskSaturatedInflow = () => [
  ...
  vol({ x0: 0.515, x1: 0.565, y0: -0.050, y1: -0.002, color: rgba('#57534e', 0.94), z0: ZOBJ - 0.045, z1: ZOBJ + 0.045, at: 0, until: 2 }),
  vol({ x0: 0.515, x1: 0.565, y0: -0.050, y1: -0.002, color: rgba('#0284c7', 0.78), z0: ZOBJ - 0.045, z1: ZOBJ + 0.045, pattern: 3, at: 3 }),
  // 빗물받이 — 배수 용량의 화신. 2단계에서 삼키다 3단계에서 역류한다.
  // 3단계엔 수면(0.042) 한참 아래라 **물속**이다 ⇒ alpha를 낮춰 물이 비쳐 오르게 한다.
  vol({ x0: 0.600, x1: 0.640, y0: -0.044, y1: 0.007, color: rgba('#44403c', 0.96), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, at: 0, until: 2 }),
  vol({ x0: 0.600, x1: 0.640, y0: -0.044, y1: 0.007, color: sunk(rgba('#44403c', 0.96)), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, at: 3 }),
  ...
  ...flow({ from: [0.620, H(0.11), ZOBJ], dir: [0, -1, 0], travel: 0.075, count: 1, scale: 0.038, color: rgba('#0369a1', 0.9), at: 2, until: 2, speed: 0.6 }),
  ...
  ...flow({ from: [0.620, H(0.03), ZOBJ], dir: [0, 1, 0], travel: 0.065, count: 2, scale: 0.04, color: rgba('#075985', 0.98), at: 3, speed: 0.7, spreadZ: 0.045 }),
