...
const floodRiskSaturatedInflow = () => [
  // 🔴 **2026-08-19 5차 — 클라이언트가 갤러리에서 직접 봤다**:
  //   *"홍수에서 물레이어 위에 건물하고 차량이 있어서 물이 가려지는게 나만 보여?"*
  //   **아니었다. 4차가 틀렸다.**
  //
  //   4차에서 저는 「물이 벽을 덮는가」만 계산하고 **물 앞에 무엇이 서 있는지**를
  //   보지 않았다. 실측하니 **물보다 카메라에 가까운 불투명 물체가 4개**였다:
  //     · 고지대 건물 2채 (alpha 0.95) — depth 3.78 · 3.93
  //     · **고지대 지형 슬래브** (alpha 0.94, 폭 0.665 × 높이 0.080) — depth 3.86
  //     · 포장면 (alpha 0.94)
  //   수면이 0.046인데 **그보다 높은 0.080 벽이 물 전체 앞을 막고 있었다.**
  //
  //   🔴 **뿌리는 배치였다 — 마른 땅을 카메라 쪽에, 잠긴 땅을 뒤에 두었다.**
  //   카메라(eye z=3.221)에서는 **z가 클수록 가깝다.** 4차는 고지대를 z=0.285에,
  //   저지대를 z=0.125에 두었으니 **보는 사람은 마른 고지대를 먼저 보고 물은 그
  //   뒤에 숨는다.** 계약이 이것을 못 잡은 이유도 같다 — 「잠겨야 할 것마다 더
  //   가까운 물 조각이 있는가」만 물었고 **「물보다 더 가까운 것이 있는가」를
  //   묻지 않았다.**
  //
  //   ⇒ **뒤집는다.** 잠긴 저지대를 **카메라 쪽(z 큰 쪽)**, 마른 고지대를
  //   **뒤(z 작은 쪽)**. 관찰자가 침수한 거리 앞에 서고 마른 고지대가 그 뒤로
  ...
  // 설명되므로 3채로 줄여도 이것은 남긴다.
  vol({ x0: 0.21, x1: 0.335, y0: 0, y1: 0.006, color: rgba('#65a30d', 0.95), pattern: 4, at: 0 }),
  // 고지대(마름) — **뒤쪽** z 0~0.125. 물이 여기 못 올라오므로 마른 대조군이고,
  // 뒤에 있으므로 **물을 가리지 않는다**(4차의 결함이 정확히 이것이었다).
  vol({ x0: 0.335, x1: 1.0, y0: 0, y1: 0.080, z0: 0, z1: 0.125,
    color: rgba('#a8a29e', 0.94), pattern: 2, at: 0 }),
  // 저지대 포장면(불투수) — **앞쪽** z 0.125~Z. 잠기는 쪽이다.
  vol({ x0: 0.335, x1: 1.0, y0: 0, y1: 0.008, z0: 0.125, z1: Z,
    color: rgba('#a8a29e', 0.94), pattern: 2, at: 0 }),
  // 고지대 줄 2채 — 뒤쪽, 마른 채. 「두 줄 = 거리」는 2채로도 성립한다.
  building({ x: 0.50, h: 0.180, w: 0.056, z: 0.06, base: 0.080, at: 0 }),
  building({ x: 0.78, h: 0.150, w: 0.052, z: 0.06, base: 0.080, at: 0, color: rgba('#a8a29e', 0.95) }),
  // 저지대 줄 3채 — **잠기는 쪽. 카메라 쪽 z=0.285.**
  // x를 0.42~0.68로 모은 이유는 물 한 판(앞막)의 중심이 세 채 전부보다 카메라에
  // 가깝게 되는 구간이기 때문이다(카메라 코드로 계산). 넓게 벌리면 오른쪽 채들이
  // 물보다 가까워져 벽이 물을 덮는다.
  building({ x: 0.42, h: 0.100, w: 0.050, z: 0.285, at: 0, color: rgba('#a8a29e', 0.95) }),
  building({ x: 0.55, h: 0.126, w: 0.054, z: 0.285, at: 0 }),
  building({ x: 0.68, h: 0.092, w: 0.046, z: 0.285, at: 0, color: rgba('#a8a29e', 0.95) }),
  // 지하 — 도시 침수가 「지하부터」인 이유(§3F: 벽·바닥으로 스미고 하수로 역류)
  vol({ x0: 0.520, x1: 0.574, y0: -0.050, y1: -0.002, color: rgba('#57534e', 0.94), z0: 0.24, z1: 0.33, at: 0 }),
  // 빗물받이 — 배수 용량의 화신. 2단계에서 삼키다 3단계에서 역류한다.
  vol({ x0: 0.640, x1: 0.680, y0: -0.044, y1: 0.007, color: rgba('#44403c', 0.96), z0: 0.245, z1: 0.325, at: 0 }),
  // 차 — **침수 깊이의 자**(조사 §Q3: 기준은 차의 창문선 60~80 cm이고 건물은
  //   자로 쓰이지 않는다). 창문선 y=0.034 · 지붕 y=0.058이라 수면 0.046에서
  //   **창문은 잠기고 지붕만 남는다.**
  //   z=0.33 — 저지대 줄(0.285)보다 카메라에 가까워 **가려지지 않고**, 앞막
  //   중심(0.3525)보다는 멀어 **물이 덮는다.** x=0.62는 두 건물 사이의 빈 x다.
  ...car({ x: 0.62, z: 0.33, at: 0 }),
  // 라벨 — `until`로 걷는다. 종전엔 전부 누적돼 3단계에 10개가 몰렸다
  // (클라이언트: *"글자들이 너무 난잡하다"*). 이 둘은 투수/불투수 **대비 쌍**이라
  // 함께 뜨고 함께 걷힌다.
  ...
  ...flow({ from: [0.660, H(0.11), 0.285], dir: [0, -1, 0], travel: 0.075, count: 1, scale: 0.038, color: rgba('#0369a1', 0.9), at: 2, speed: 0.6 }),
  ...
  vol({ x0: 0.522, x1: 0.572, y0: -0.048, y1: -0.004, color: rgba('#0284c7', 0.72), z0: 0.245, z1: 0.325, pattern: 3, at: 3 }),
  // 지표수 **2판**. 저지대 줄(z=0.285)을 경계로 뒤(본체)와 앞(앞막)으로 가른다.
  //   앞막이 **가장 나중에** 그려지며 alpha 0.5로 벽·차의 하부를 물들인다 = 수면선.
  ...
  // 3단계 라벨 **4개**. `soilAlreadyFull`은 `groundCannotAbsorb`와 같은 사실의
  // 두 표현이고 그 뜻은 캡션 4번째 문장이 갖는다 — **지운 것이 아니라 캡션이 소유한다.**
