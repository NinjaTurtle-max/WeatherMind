...
const floodRiskSaturatedInflow = () => [
  ...
  // ⓑ **잠긴 정도를 집집마다 다르게 한다**(참조 규약). 참조에서 깊이를 읽게 하는
  //   것은 물 색이 아니라 **「지붕만 남은 집」과 「벽 절반인 집」이 함께 있는 것**이다.
  //   h 0.100 → **0.068**: 수면 0.042가 **62%**를 덮어 지붕 쪽만 남는다.
  //   다른 두 채는 33%(h 0.126)·46%(h 0.092)라 **세 단계 깊이**가 한 화면에 있다.
  //   ⚠️ h를 0.06 이하로 내리지 말 것 — 「침수 깊이의 자」 계약이 높이 0.06 이하를
  //      **차 크기 물체**로 세므로 건물이 자로 오인된다.
  building({ x: 0.42, h: 0.068, w: 0.050, z: ZOBJ, at: 0, color: rgba('#a8a29e', 0.95) }),
  building({ x: 0.55, h: 0.126, w: 0.054, z: ZOBJ, at: 0 }),
  building({ x: 0.68, h: 0.092, w: 0.046, z: ZOBJ, at: 0, color: rgba('#a8a29e', 0.95) }),
  // 지하 — 도시 침수가 「지하부터」인 이유(§3F: 벽·바닥으로 스미고 하수로 역류)
  vol({ x0: 0.520, x1: 0.574, y0: -0.050, y1: -0.002, color: rgba('#57534e', 0.94), z0: ZOBJ - 0.045, z1: ZOBJ + 0.045, at: 0 }),
  // 빗물받이 — 배수 용량의 화신. 2단계에서 삼키다 3단계에서 역류한다.
  vol({ x0: 0.640, x1: 0.680, y0: -0.044, y1: 0.007, color: rgba('#44403c', 0.96), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, at: 0 }),
  ...
  vol({ x0: 0.522, x1: 0.572, y0: -0.048, y1: -0.004, color: rgba('#0284c7', 0.72), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, pattern: 3, at: 3 }),
  ...
  //   투수/불투수 대비를 부쉈다. 색은 8차 값 그대로(`#0891b2` 0.58 — 참조 도판의
  //   진한 청록). y0 -0.030은 **몸통과 이어 붙이는 값**이다: 지표 아래로 물이 차
  //   있는 것이 포화(§3C)이고, 잘린 면에서 청록 층의 두께로 보인다.
  //   ⚠️ **수면(y1)은 올리지 않는다** — H(0.105)는 앞줄 건물을 3~6층으로 읽으면
  //      실척 3.6~10 m이고 한국 내수침수위험지도 최상위 밴드(3.0 m 이상)를 이미 넘는다.
  vol({ x0: 0.335, x1: 0.98, y0: -0.030, y1: H(0.105), z0: 0.14, z1: Z,
    color: rgba('#0891b2', 0.58), pattern: 3, at: 3, layer: 'ground' }),
