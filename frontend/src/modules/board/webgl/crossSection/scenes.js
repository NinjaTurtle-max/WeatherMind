...
const floodRiskSaturatedInflow = () => [
  // 🔴 **2026-08-19 4차 재작성 — 클라이언트: *"레이어를 너무 많이 쌓아서 그래"*.**
  //
  //   3차까지 저는 「무엇을 더 넣을까」로만 움직였다: 2차에 차·고지대를 넣고,
  //   3차에 물 상자를 6조각으로 쪼갰다. **진단은 맞았지만 방향이 틀렸다** —
  //   `renderer.js`는 `gl.disable(gl.DEPTH_TEST)`로 **화가 알고리즘**을 쓰고
  //   물체를 **중심 한 점**으로 정렬한다. 상자가 많으면 어느 배치도 안전하지
  //   않다. 조각을 늘려 순서를 이기는 것은 **원인을 늘려 증상을 덮은 것**이다.
  //
  //   실측이 그 지적을 뒷받침했다 — 20장면 중 이 장면이
  //     **solid 29 (2위의 3배)** · **라벨 10 (중앙값 5의 2배)**
  //   로 **둘 다 1위**였다. 「도시」를 그리려고 물건을 계속 더한 결과다.
  //
  //   ⇒ **줄여서 푼다.** 저지대 5채 → **3채**(좁은 x에 모아 물 한 판이 정렬을
  //   이긴다) · 고지대 4채 → **2채**(「두 줄 = 거리」는 2채로도 성립) ·
  //   차 2대 → **1대**(자는 하나로 충분하고, 종전엔 2대 중 1대가 물 위에
  //   그려져 절반만 작동했다) · 물 6조각 → **2판**(본체 + 앞막).
  //     solid **29 → 19** · 라벨 누적 **10 → 최대 5**(단계별 [3,5,2,4]).
  //
  //   ⚠️ **물의 y는 여전히 한 값도 안 바뀌었다.** 수면은 H(0.115)=0.046이고
  //      실척 3.6~10 m다(조사 §Q2 — 한국 침수 실적 최상위 밴드 3.0 m 초과).
  //      「건물 일부가 잠기게」는 **물을 올려서가 아니라 잠긴 것이 보이게** 해서
  //      성립시킨다.
  //
  // ── 지형 — 투수/불투수 대비가 이 장면의 핵심이다(조사 §3F ④).
  //    「왜 도시에서만 잠기나」는 이 대비로만 설명된다. 그래서 3채로 줄여도
  //    풀밭·포장면·고지대 셋은 남긴다.
  ...
  // 고지대 — z 0.20~Z. 물이 여기 못 올라오므로 뒷줄은 마른 채다(전부 수몰 아님).
  vol({ x0: 0.335, x1: 1.0, y0: 0, y1: 0.080, z0: 0.20, z1: Z,
    color: rgba('#a8a29e', 0.94), pattern: 2, at: 0 }),
  // 고지대 줄 2채 — 카메라에 가장 가깝다(z가 클수록 가깝다). 마른 대조군.
  building({ x: 0.50, h: 0.180, w: 0.056, z: ZC + 0.075, base: 0.080, at: 0 }),
  building({ x: 0.78, h: 0.150, w: 0.052, z: ZC + 0.075, base: 0.080, at: 0, color: rgba('#a8a29e', 0.95) }),
  // 저지대 줄 3채 — **잠기는 쪽**. x를 0.42~0.68로 모은 이유는 물 한 판(앞막)의
  // 중심이 세 채 전부보다 카메라에 가깝게 되는 구간이기 때문이다(카메라 코드로
  // 계산). 넓게 벌리면 오른쪽 채들이 물보다 가까워져 벽이 물을 덮는다 —
  // 3차에서 5채 중 3채가 그 상태였다.
  building({ x: 0.42, h: 0.100, w: 0.050, z: ZC - 0.085, at: 0, color: rgba('#a8a29e', 0.95) }),
  building({ x: 0.55, h: 0.126, w: 0.054, z: ZC - 0.085, at: 0 }),
  building({ x: 0.68, h: 0.092, w: 0.046, z: ZC - 0.085, at: 0, color: rgba('#a8a29e', 0.95) }),
  // 지하 — 도시 침수가 「지하부터」인 이유(§3F: 벽·바닥으로 스미고 하수로 역류)
  vol({ x0: 0.520, x1: 0.574, y0: -0.050, y1: -0.002, color: rgba('#57534e', 0.94), z0: ZC - 0.13, z1: ZC - 0.04, at: 0 }),
  // 빗물받이 — 배수 용량의 화신. 2단계에서 삼키다 3단계에서 역류한다.
  vol({ x0: 0.640, x1: 0.680, y0: -0.044, y1: 0.007, color: rgba('#44403c', 0.96), z0: ZC - 0.125, z1: ZC - 0.045, at: 0 }),
  // 차 — **침수 깊이의 자**(조사 §Q3: 침수 보도가 쓰는 기준은 차의 창문선
  //   60~80 cm이고, 건물은 자로 쓰이지 않는다). 창문선 y=0.034 · 지붕 y=0.058이라
  //   수면 0.046에서 **창문은 잠기고 지붕만 남는다.**
  //   z=0.14 — 저지대 줄(0.125)보다 카메라에 가까워 **가려지지 않고**, 앞막
  //   중심(0.1625)보다는 멀어 **물이 덮는다.** x=0.62는 두 건물 사이의 빈 x다.
  ...car({ x: 0.62, z: 0.14, at: 0 }),
  // 라벨 — `until`로 걷는다. 종전엔 전부 누적돼 3단계에 10개가 몰렸다
  // (클라이언트: *"글자들이 너무 난잡하다"*). 이 두 개는 투수/불투수 **대비 쌍**이라
  // 함께 뜨고 함께 걷힌다.
  ...
  ...flow({ from: [0.660, H(0.11), ZC - 0.085], dir: [0, -1, 0], travel: 0.075, count: 1, scale: 0.038, color: rgba('#0369a1', 0.9), at: 2, speed: 0.6 }),
  ...
  vol({ x0: 0.522, x1: 0.572, y0: -0.048, y1: -0.004, color: rgba('#0284c7', 0.72), z0: ZC - 0.125, z1: ZC - 0.045, pattern: 3, at: 3 }),
  // 지표수 **2판**. 저지대 줄(z=0.125)을 경계로 뒤(본체)와 거리(앞막)로 가른다.
  //   본체 — 줄 뒤편 + 풀밭 경계까지. 앞막 — 저지대 줄과 고지대 사이의 거리.
  //   앞막이 **나중에** 그려지며 alpha 0.5로 벽·차의 하부를 물들인다 = 수면선이 보인다.
  //   ⚠️ z는 0.20에서 멎는다 — 고지대를 관통하지 않는다.
  ...
  vol({ x0: 0.335, x1: 0.98, y0: -0.006, y1: H(0.115), z0: 0.125, z1: 0.20,
    color: rgba('#38bdf8', 0.5), pattern: 3, at: 3 }),
  // 빗물받이 역류 — 아래로 못 내려가니 위로 되올라온다
  ...flow({ from: [0.660, H(0.03), ZC - 0.085], dir: [0, 1, 0], travel: 0.065, count: 2, scale: 0.04, color: rgba('#075985', 0.98), at: 3, speed: 0.7, spreadZ: 0.045 }),
  ...
  // 3단계 라벨 **4개**. `soilAlreadyFull`(땅이 이미 찼다)은 뺐다 —
  // `groundCannotAbsorb`(땅이 못 흡수한다)와 **같은 사실의 두 표현**이고,
  // 그 뜻은 캡션 4번째 문장이 이미 갖고 있다(*"땅이 스며들 수 있는 양을 넘겨
  // 물이 고이기 시작해요"*). **지운 것이 아니라 캡션이 소유한다.**
  // ⚠️ 리소스 키는 남는다 — SVG 폴백(`CrossSectionPanel.jsx`)이 여전히 쓴다.
