...
const floodRiskSaturatedInflow = () => [
  ...
  // **고지대**(2026-08-19 조사 §Q2) — 뒷줄이 선 단(段)이다. 「낮은 곳부터」는 색도
  // 투명도도 아니고 **경사진 지면선과 수평 수면의 교점** 하나로 말해진다. 종전에는
  // 전 건물이 `y0: 0`이라 **그 문장을 표현할 수단 자체가 없었다.**
  // 높이 0.080은 3단계 수면(0.046)보다 위다 — 그래서 뒷줄은 **마른 채로 남는다.**
  vol({ x0: 0.335, x1: 1.0, y0: 0, y1: 0.080, z0: 0.20, z1: Z,
    color: rgba('#a8a29e', 0.94), pattern: 2, at: 0 }),
  // 두 줄로 세운다 — 한 줄이면 「건물 몇 개」이지 도시가 아니다. 뒷줄은 **고지대**에,
  // 앞줄은 **저지대**에 두어 그 사이가 **비탈진 거리**로 읽히게 하고, 낮은 쪽에
  // 빗물받이를 둔다. 물이 어디로 모이는지가 지형만으로 정해진다.
  building({ x: 0.44, h: 0.160, w: 0.052, z: ZC + 0.075, base: 0.080, at: 0 }),
  building({ x: 0.575, h: 0.200, w: 0.056, z: ZC + 0.075, base: 0.080, at: 0, color: rgba('#a8a29e', 0.95) }),
  building({ x: 0.715, h: 0.145, w: 0.050, z: ZC + 0.075, base: 0.080, at: 0 }),
  building({ x: 0.855, h: 0.185, w: 0.058, z: ZC + 0.075, base: 0.080, at: 0, color: rgba('#a8a29e', 0.95) }),
  ...
  // **자**(§Q1) — 앞줄 건물 사이, 카메라에 가장 가까운 z. 창문선 y=0.034 ·
  // 지붕 y=0.058이고 3단계 수면이 0.046이라 **창문은 잠기고 지붕만 남는다.**
  // 빗물받이(x 0.700~0.740)를 피해 배치한다.
  ...car({ x: 0.455, z: ZC - 0.15, at: 0 }),
  ...car({ x: 0.855, z: ZC - 0.15, at: 0, body: rgba('#334155', 0.97) }),
  ...
  // ⚠️ **`until: 2` — 3단계에서 끊는다**(2026-08-19 조사 §Q3). 홍수와 해일의 갈림은
  //    깊이가 아니라 **물의 거동**이다: 홍수는 **수평·무결한 면이 제자리에서** 오르고,
  //    해일은 **전선을 가진 파형이 바다에서 밀려온다.** 이 장면은 서쪽 0~0.2가
  //    바다인데, 서→동 화살표가 침수 단계까지 살아 있으면 **물이 바다에서 밀려오는
  //    것으로 읽힌다.** 유입은 0~2단계가 소유하고, 3단계의 물은 **지면과
  //    빗물받이에서** 올라온다(아래 역류 화살표가 그것이다).
  ...
  // 지표수 — **도시 저지대에만** 고인다. 셋을 동시에 좁힌다:
  //   · `x0: 0.335` — 종전 `0.02`는 **바다(서쪽 0~0.2)에 붙어** 물이 거기서 이어져
  //     보였다. 유입 화살표를 `until: 2`로 끊고도 **물 자체가 해일로 읽히는** 자리다.
  //     또 종전 값은 **풀밭(0.21~0.335)까지 덮어**, 바로 옆 「풀밭은 스며든다」 라벨과
  //     조사가 *"④가 핵심"*이라 한 **투수/불투수 대비**를 정면으로 부쉈다.
  //   · `z1: 0.20` — 뒷줄 고지대(z 0.20~Z)를 **관통하지 않게**. 종전엔 z 인자가 없어
  //     전체 Z였고, 그러면 고지대 볼륨 속으로 물이 들어간다.
  //   이 셋이 「낮은 곳부터」를 지형만으로 성립시킨다 — 물은 수평·무결하게 두고
  //   지면이 기울어 있다(조사 §Q2·§Q3).
  //
  // 🔴 **2026-08-19 3차: 「건물 일부가 물에 잠기게」**(클라이언트 직접 지시).
  //   2차에서 저는 *"수위는 안 올린다 — 이미 실척 3.6~10 m로 한국 최상위 밴드
  //   초과이고, 안 읽힌 이유는 잴 물건이 없어서다"*라고 판정했고 차·고지대를
  //   넣었다. **그 판정으로는 부족하다는 것이 클라이언트 판단이다.**
  //
  //   ⚠️ **그런데 수위를 올리는 것은 여전히 답이 아니다.** 형상을 재면 앞줄은
  //   **이미 높이의 36~51%가 물에 담겨 있다**(수면 H(0.115)=0.046 · base 0 ·
  //   h 0.090~0.128 · 물의 z범위 안). 기하학적으로 「일부 잠김」은 이미 참이었다.
  //
  //   🔴 **진짜 원인은 렌더러의 합성 순서다.** `renderer.js`는
  //   `gl.disable(gl.DEPTH_TEST)`로 **화가 알고리즘**을 쓰고, 물체를 **중심
  //   깊이 하나로** 정렬해 통째로 앞뒤를 정한다(`solids.sort`). 큰 물 상자
  //   하나는 각 건물보다 **전부 앞이거나 전부 뒤**일 수밖에 없고, 카메라
  //   (방위 36°·고도 21°, eye z=3.221)에서는 **z가 큰 쪽이 가깝다.**
  //   저지대 줄은 z=0.125, 종전 물 상자 중심은 z=0.10 ⇒ **건물이 물보다
  //   가깝다** ⇒ 불투명한 벽(alpha 0.94~0.97)이 물을 덮어 **수면선이 벽에
  //   생기지 않는다.** 실측: 앞줄 5채 중 **3채**(x 0.635·0.795·0.915)와
  //   **차 2대 중 1대**(x 0.855)가 그 상태였다 — **깊이를 재는 자 자체가
  //   절반만 작동했다.**
  //
  //   ⇒ **물을 올리지 않고 상자를 쪼갠다.** 저지대 줄(z=0.125)을 기준으로
  //   뒤(본체)와 앞(거리)으로 z를 가르고, **앞막은 x로도 5조각**으로 나눠
  //   각 조각의 중심이 대응 건물보다 카메라에 가깝게 만든다. 그러면 앞막이
  //   **나중에** 그려지며 alpha 0.5로 벽 하부를 물들인다 = 수면선이 보인다.
  //   실측 결과 **잠겨야 할 9개 전부**(건물 5 + 차체 2 + 캐빈 2)가 덮인다.
  //
  //   ⚠️ **물의 y·수량은 한 값도 안 바뀌었다** — 실척 근거는 그대로 유효하다.
  //      바뀐 것은 **같은 물을 어느 순서로 그리는가**뿐이다.
  //   ⚠️ 앞막의 z는 `0.20`에서 멎는다 — 고지대(z 0.20~Z)를 관통하지 않는다.
  //      뒷줄(z=0.285)은 여전히 마른 채다. 「전부 수몰」이 아니다.
  //   ⚠️ 조각들은 **정확히 맞닿는다**(겹치면 alpha가 두 번 곱해져 이음매가
  //      진해진다). y1은 물 상자 밖을 안 그리므로 **차 지붕(y 0.058)은 남는다.**
  //
  // 본체 — 저지대 줄 **뒤편** + 차가 선 자리(z 0.06)
  vol({ x0: 0.335, x1: 0.98, y0: -0.006, y1: H(0.115), z0: 0, z1: 0.125,
    color: rgba('#38bdf8', 0.5), pattern: 3, at: 3 }),
  // 앞막 — 저지대 줄과 고지대 사이의 **거리**에 고인 물. x 5조각으로 나눈 이유는
  // 위 ⇒ 문단(중심 깊이 정렬을 건물마다 이기게 하려고)이지 물리가 아니다.
  ...[[0.335, 0.47], [0.47, 0.59], [0.59, 0.71], [0.71, 0.86], [0.86, 0.98]].map(
    ([x0, x1]) => vol({ x0, x1, y0: -0.006, y1: H(0.115), z0: 0.125, z1: 0.20,
      color: rgba('#38bdf8', 0.5), pattern: 3, at: 3 })),
