 ...
 *
 * 🔴 **`waterY` — 수면선을 차 위에 긋는다**(2026-08-19 10차).
 * 물이 몸통의 면(`ground`)이 되면 물은 차보다 **항상 먼저** 그려지므로 덮지 못하고,
 * 물에서 두께를 뺐으므로 **쌓아서** 잠기게 할 수도 없다. 남는 수는 조사 §2⑴이
 * 관례라고 적은 것 하나다:
 *   *"수면선 위는 원색 그대로, 아래만 물빛으로 물든다 — 윤곽은 계속 비친다"*
 * ⇒ 수면 y에서 **조각을 가르고 아래쪽을 물빛으로 물들인다.** 조각의 경계선 자체가
 *   **수면선**이고, 건물 세 채가 같은 높이에서 같은 선을 가지므로 도시를 가로지르는
 *   **한 줄**이 생긴다.
 * ⚠️ 차만은 조각을 **지우면 안 된다** — 수면 위에 남는 것이 캐빈 윗동강 0.016뿐이라
 *    차로 안 읽히고, 그러면 **깊이를 잴 물건이 사라진다**(2차 반려의 바로 그 사유).
 *    창문선 0.034 · 지붕 0.058 · 수면 0.042 ⇒ 「창문까지 잠기고 지붕만 남았다」가
 *    **캐빈 한복판을 가르는 선**으로 보인다. 이것이 자가 하는 일이다.
 */
function car({ x, z = ZC - 0.085, at, until, waterY = null,
  body = rgba('#475569', 0.97), cabin = rgba('#94a3b8', 0.95) }) {
  const W = 0.070;   // 차체 길이
  const Hb = 0.030;  // 차체(바퀴~창문 아래)
  const Hc = 0.024;  // 캐빈(창문~지붕) — 창문선 = 0.004 + Hb = 0.034
  const sill = 0.004 + Hb;
  const roof = sill + Hc;
  const chassis = (y0, y1, color) => vol({ x0: x - W / 2, x1: x + W / 2, y0, y1,
    z0: z - 0.026, z1: z + 0.026, color, at, until });
  const cab = (y0, y1, color) => vol({ x0: x - W * 0.30, x1: x + W * 0.30, y0, y1,
    z0: z - 0.024, z1: z + 0.024, color, at, until });
  if (waterY === null) return [chassis(0.004, sill, body), cab(sill, roof, cabin)];
  return [
    chassis(0.004, sill, sunk(body)),
    cab(sill, waterY, sunk(cabin)),
    cab(waterY, roof, cabin),
  ];
}
...
/**
 * 🔴 **홍수 도시 물체의 단일 깊이**(2026-08-19 9차, 클라이언트: *"물하고 건물하고
 * 자동차를 한 레이어에 넣어. 지금 레이어를 쌓아서 부정합하니"*).
 *
 * 종전엔 건물 z=0.22 · 차 z=0.25로 **깊이가 갈려** 있었다. `renderer.js`가
 * `gl.disable(gl.DEPTH_TEST)`로 **중심 깊이 하나로** 정렬하므로, 깊이가 갈리면
 * **물체마다 앞뒤가 따로 정해지고** 물과의 관계도 물체마다 달라진다 — 그것이
 * 「레이어를 쌓아서 부정합」의 정체다.
 *
 * ⇒ 도시 물체(건물 3채 · 차 · 지하 · 빗물받이)를 **이 한 값**에 모은다.
 *   물만 그 앞에 온다: 물 z `0.14~Z`(중심 0.28)가 물체 띠(0.155~0.245)를 **감싸면서**
 *   중심이 더 앞이라 **가장 나중에** 그려진다. 실측 여유 **+0.034**.
 *   ⚠️ 차 x 0.585~0.655는 건물 x 세 구간과 **겹치지 않는다** — 같은 깊이에 둬도
 *      서로 관통하지 않는다(실측 확인).
 *   ⚠️ 이 값을 키우면 건물 x=0.68이 물보다 앞으로 나온다(0.24에서 실측 -0.012).
 */
const ZOBJ = 0.20;
...
const FLOOD_CITY = [
  { x: 0.42, h: 0.068, w: 0.050, color: rgba('#a8a29e', 0.95) },
  { x: 0.55, h: 0.126, w: 0.054, color: rgba('#94a3b8', 0.95) },
  { x: 0.68, h: 0.092, w: 0.046, color: rgba('#a8a29e', 0.95) },
];
...
const floodRiskSaturatedInflow = () => [
  ...
  vol({ x0: 0.335, x1: 1.0, y0: -0.014, y1: 0.002, color: rgba('#a8a29e', 0.94), pattern: 2, at: 0, layer: 'ground' }),
  ...
  ...FLOOD_CITY.map((b) => building({ ...b, z: ZOBJ, at: 0, until: 2 })),
  ...FLOOD_CITY.flatMap((b) => [
    building({ ...b, h: WATERLINE, z: ZOBJ, at: 3, color: sunk(b.color) }),
    building({ ...b, h: b.h - WATERLINE, base: WATERLINE, z: ZOBJ, at: 3 }),
  ]),
  ...
  vol({ x0: 0.520, x1: 0.574, y0: -0.050, y1: -0.002, color: rgba('#57534e', 0.94), z0: ZOBJ - 0.045, z1: ZOBJ + 0.045, at: 0, until: 2 }),
  vol({ x0: 0.520, x1: 0.574, y0: -0.050, y1: -0.002, color: rgba('#0284c7', 0.78), z0: ZOBJ - 0.045, z1: ZOBJ + 0.045, pattern: 3, at: 3 }),
  // 빗물받이 — 배수 용량의 화신. 2단계에서 삼키다 3단계에서 역류한다.
  // 3단계엔 수면(0.042) 한참 아래라 **물속**이다 ⇒ alpha를 낮춰 물이 비쳐 오르게 한다.
  vol({ x0: 0.640, x1: 0.680, y0: -0.044, y1: 0.007, color: rgba('#44403c', 0.96), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, at: 0, until: 2 }),
  vol({ x0: 0.640, x1: 0.680, y0: -0.044, y1: 0.007, color: sunk(rgba('#44403c', 0.96)), z0: ZOBJ - 0.04, z1: ZOBJ + 0.04, at: 3 }),
  // 차 — **침수 깊이의 자**(조사 §Q3). 창문선 0.034 · 지붕 0.058 · 수면 0.042 ⇒
  //   **창문까지 잠기고 지붕만 남는다.** 3단계에 수면에서 조각이 갈리고 아래쪽만
  //   물빛으로 물든다(§2⑴ *"수면선 위는 원색 그대로, 아래만 물든다"*).
  ...car({ x: 0.62, z: ZOBJ, at: 0, until: 2 }),
  ...car({ x: 0.62, z: ZOBJ, at: 3, waterY: WATERLINE }),
  ...
  ...flow({ from: [0.660, H(0.11), ZOBJ], dir: [0, -1, 0], travel: 0.075, count: 1, scale: 0.038, color: rgba('#0369a1', 0.9), at: 2, speed: 0.6 }),
  ...
  vol({ x0: 0, x1: 1, y0: -0.066, y1: -0.022, z0: 0.10, z1: Z, color: rgba('#0ea5e9', 0.34), pattern: 3, at: 3, layer: 'ground' }),
  ...
  //   갖는다(`WATERLINE` 참조).
  //
  //   ⓐ **깊이는 높이가 아니라 색 계조**(조사 정정 2). 가운데가 진하고 가장자리가
  //     옅다 — 두 장을 겹치되 **진한 쪽이 반드시 나중에** 그려져야 한다. `ground`도
  //     자기들끼리는 중심 깊이로 정렬되므로(`renderer.js` rebuild) 그 순서는
  //     좌표가 정한다: 실측 nearness **옅음 0.5694 < 진함 0.5944**(여유 0.025).
  //     ⚠️ 이 대소가 뒤집히면 **계조가 사라지는 것이 아니라 반대로 뒤집힌다** —
  //        계약이 그 대소를 직접 문다(7-g).
  //   ⓑ 진한 쪽만 `y0`를 더 깊이(-0.026) 내린다. 윗면은 같은 높이라 화면에 턱이
  //     안 생기고, **앞 잘린 면에서만 청록 층이 두꺼워진다** = 「가운데가 깊다」.
  //   ⓒ x0=0.335 — 종전 0.02는 바다(0~0.2)에 붙어 **해일로 읽히고** 풀밭까지 덮어
  //     투수/불투수 대비를 부쉈다. 색은 8차 값(`#0891b2`) 그대로.
  vol({ x0: 0.335, x1: 0.98, y0: -0.018, y1: 0.002, z0: 0.14, z1: Z,
    color: rgba('#0891b2', 0.42), pattern: 3, at: 3, layer: 'ground' }),
  vol({ x0: 0.40, x1: 0.90, y0: -0.026, y1: 0.004, z0: 0.22, z1: Z,
    color: rgba('#0891b2', 0.72), pattern: 3, at: 3, layer: 'ground' }),
  // 빗물받이 역류 — 아래로 못 내려가니 위로 되올라온다
  ...flow({ from: [0.660, H(0.03), ZOBJ], dir: [0, 1, 0], travel: 0.065, count: 2, scale: 0.04, color: rgba('#075985', 0.98), at: 3, speed: 0.7, spreadZ: 0.045 }),
