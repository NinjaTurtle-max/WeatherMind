...
const SPIN_PER_FRAME = 1 / 15;

// 태풍 중심(뷰 좌표 0~1) · 반경(뷰 폭 대비). 한반도 남서 해상 — 실제 접근 구도.
const EYE = { x: 0.38, y: 0.62, r: 0.42 };

/**
 * 강조 IR 색 램프 (MT-21 개정 ③) — 구름 **꼭대기 온도**를 색으로 읽는 관례.
 *
 * 회색조에서 바꾼 이유: 실제 예보 현장에서 쓰는 강조 IR은 가장 찬(=가장 높은)
 * 구름만 색으로 튀게 만든다. 그래야 "이 태풍의 어디가 제일 발달했나"가 한눈에 든다 —
 * 회색조로는 두꺼운 구름과 아주 두꺼운 구름이 둘 다 흰색이라 구분이 안 된다.
 *
 * 색 순서는 관례를 따른다: 바다(검정) → 얇은 구름(회청) → 흰색 → 초록 → 노랑 →
 * 빨강 → 자홍. **자홍이 가장 차고 가장 높은 꼭대기**다.
 */
const IR_RAMP = [
  [0.00, [6, 14, 28]],     // 바다
  [0.22, [58, 78, 104]],   // 얇은 하층운
  [0.44, [206, 216, 232]], // 중층운
  [0.58, [255, 255, 255]], // 두꺼운 구름
  [0.70, [64, 200, 120]],  // 초록 — 여기부터 강조 구간
  [0.80, [242, 214, 72]],  // 노랑
  [0.90, [232, 92, 60]],   // 빨강
  [1.00, [196, 74, 198]],  // 자홍 — 가장 찬 꼭대기
];
...
export default function SatelliteView({ intensity, shear }) {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      setSpin((v) => (v + SPIN_PER_FRAME) % 1);          // 외곽 한 바퀴 ≈ 2.5초
