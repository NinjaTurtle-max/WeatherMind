 ...
 * 종전에는 이 자리가 *"단계는 누적이다 — 마지막에 수지 전체가 한 화면에 있어야
 * 「균형」이 보인다"*고만 적었고, 그 결과 **4단계에 라벨 15개**가 한꺼번에 떴다
 * (실측 2,5,7,14,15). 클라이언트 지적이 정확히 그것이다 — *"글자들이 너무
 * 난잡하다"*. 그래서 **화살표·구름·층(그림)은 그대로 누적**하되 **역할이 끝난
 * 라벨만 `until`로 걷는다.** 「균형」은 여전히 보인다: 4단계에 흡수 쪽(51·14)과
 * 방출 쪽(17·48)이 **숫자로 함께 남아 있다.** 걷히는 것은 앞 단계에서 이미
 * 읽힌 반사 3종과, 캡션이 대신 말하는 설명 문구뿐이다.
 *
...
const RAW_ITEMS = [
  ...
  // ⚠️ 반사 3종 라벨은 `until: 2`다 — 2단계까지 남아 **35(되나감) ↔ 65(흡수)**가
  //    나란히 읽히게 하고, 3단계(나가는 열)부터는 이야기에서 빠지므로 걷는다.
  ...
  // ⚠️ **층에 붙던 이름표(「온실기체 층」)를 캔버스에서 걷었다**(2026-08-19). 층은
  //    반투명 볼륨으로 그대로 서 있고, 이름은 3단계 캡션이 부른다("…온실기체 층이
  //    다시 아래로도 내보내는 것이 온실효과다"). 무대를 소개하는 문구는 캡션 몫이라는
  //    보드 문법 그대로다 — 캔버스에는 **되돌아오는 화살표와 그 이름**만 남는다.
  vol({ x0: 0, x1: 1, y0: H(GHG_LO), y1: H(GHG_HI), color: rgba('#38bdf8', 0.2), at: 3 }),
  // 지표 → 대기 34: 셋을 나란히 세워 「잠열이 제일 굵다」가 보이게 한다.
  // 🔴 **갈래 셋에 라벨 셋을 붙이던 것을 합계 하나로 합쳤다**(2026-08-19 가독성 지적).
  //    3단계는 종전에 라벨 7개가 한꺼번에 붙던 자리였고 그중 셋이 여기였다.
  //    **수치를 지운 것이 아니라 옮겼다** — 갈래별 내역(19·9·6)은 이 단계 `note`가
  //    말하고, 캔버스에는 수지 식에 그대로 쓰이는 **합계 34**가 남는다
  //    (51 = 17 + 34가 화면에서 검산된다). 굵기 차이(잠열이 제일 굵다)는 화살표가
  //    이미 말하므로 라벨 셋이 그것을 되풀이할 필요가 없다.
