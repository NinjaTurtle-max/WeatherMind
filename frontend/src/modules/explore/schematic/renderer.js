 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 아래 형식만 계약이고, 장면은 별도 파일이 만들어 `setScene`에 넘긴다.
 * **지금 서 있는 장면은 C1(복사수지) 하나**이고 `radiationScene.js`가 소유한다 —
 * T1(태풍 단면)·T2(태풍 생애)는 **장면 데이터만** 더 쓰면 되고 렌더러는 그대로다
 * (MT-22를 「1종 먼저 세우고 실기기로 확인」으로 자른 클라이언트 지시).
 *
 * ```js
 * const scene = {
...
import { schematicCamera, projectToPercent, ASPECT } from './camera.js';
import { arrowMesh3D, arrow3DVertexCount, arrowBasis, ARROW3D_DEFAULTS } from './arrowMesh.js';
...
export function createSchematicRenderer(canvas, meshOpts = null) {
  ...
  let step = 0;
  let dirty = true;
  // ⚠️ 종횡비는 **cam과 따로** 들고 있는다. 파킹본은 `cam.aspect ?? ASPECT`로
  // 되읽었는데 `schematicCamera()`가 돌려주는 객체에는 aspect가 없어서(resize가
  // 나중에 얹는다) **setScene을 부른 순간 카메라가 16:10으로 되돌아갔다** —
  // 다음 resize가 올 때까지 세로가 긴 패널에서 그림이 눌린다. 상태를 한 곳에 둔다.
  let aspect = ASPECT;
  let cam = schematicCamera(aspect);
  ...
  function resize() {
    ...
    aspect = (rect.width || 1) / Math.max(rect.height || 1, 1);
    cam = schematicCamera(aspect, scene?.camera);
  ...
  return {
    ...
    /** 라벨 겹쳐 그리기(`labelsFor`)가 같은 종횡비를 써야 위치가 맞는다 */
