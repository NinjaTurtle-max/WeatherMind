...
const BADGE_ICON = {
  streak_7: '🔥',
  // ⚡는 VS16(U+FE0F)을 달고 있어야 한다 — **예방이다.** 2026-08-19에 유닛 노드에서
  // 같은 자리를 닫으며 여기도 함께 닫았다(한 자리만 고치면 N곳 중 N-1을 고친 것이다).
  // 🔴 **유니코드 속성으로 판정하면 이 글자는 「문제없다」가 나온다** — U+26A1은
  // `Emoji_Presentation=Yes`다(Node·파이썬 양쪽 실측). 원인이 속성이 아니라 브라우저가
  // 기본 표현보다 **CSS 폰트 스택을 먼저 걷는** 것이기 때문이다.
  // ⚠️ **이 맥에서는 결함이 재현되지 않는다**(2026-08-19 재실측 — 실제 스택 아래에서
  // bare와 VS16이 픽셀까지 동일). 종전에 이 자리에 적었던 「잉크가 `24×17.9`로
  // 떨어진다」는 **폰트를 명시했을 때의 값**이었다. 근거와 경위는
  // `unitIcon.js`의 `anomaly` 주석과 `learnPath.smoke.test.mjs` ⑪-c가 소유한다.
  streak_30: '⚡️',
  streak_100: '💯',
  perfect_session: '🌈',
  tier_promoted: '🎖️',
