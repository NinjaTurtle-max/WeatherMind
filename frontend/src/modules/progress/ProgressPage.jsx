      ...
      <div className="grid grid-cols-[minmax(0,1fr)] gap-4 lg:grid-cols-2">
        ...
        <div className="contents lg:flex lg:flex-col lg:gap-4">
          ...
          {/* lg:flex-1 — 오른쪽 열이 더 길 때 그 차이를 **이 칸**이 먹는다
              ...
              내려보낸다 — 짝을 이루는 코드다). */}
          {/* 🔴 **흡수(`lg:flex-1`)를 되살렸다**(2026-08-19 저녁 사용자 지시 —
              "배지 컬렉션 세로 길이를 더 늘려서 여백 맞춰줘").

              하루 사이 이 한 클래스가 **두 번 뒤집혔다.** 경위를 남기는 이유는
              두 지시가 서로 모순이 아니라 **그때그때 화면이 달랐기 때문**이다:
              · 오전 「배지 카드 크기는 원래대로」 — 그때는 학습 지역이 오른쪽에
                있어 왼쪽이 크게 짧았고, 배지가 그 큰 차이를 먹어 타일이 눈에 띄게
                늘어났다. 남는 자리는 **능력 분석 판의 탭**이 먹기로 했다.
              · 그 뒤 탭도 걷혔고(사용자 지시) 학습 지역도 왼쪽으로 돌아왔다.
                그래서 지금 남는 차이는 **58px뿐**이고(실측 왼쪽 바닥 807 ↔
                오른쪽 865), 그것을 먹을 칸이 아무 데도 없어 흰 여백으로 남았다.
              흡수 칸이 배지인 이유는 종전과 같다: 타일 5장이 나눠 받아 커져도
              빈 데가 없다. 학습 지역으로 옮기면 한 줄짜리 카드가 늘어 빈 카드가
              된다(2026-08-12에 142px까지 늘렸다가 되돌린 자리 — 아래 order-7).
              ⚠️ `BadgeCollection`의 `h-full`·격자 `flex-1`과 **짝**이다. 여기만
                 주면 카드만 커지고 타일은 그대로라 그 차이가 흰 여백이 된다. */}
          <div className="order-2 lg:flex-1 lg:order-none">
...
function SaveProgressCard() {
  ...
  return (
    <div
      ...
      // 🔴 `mt-4`는 이 카드 몫이다 — B조의 꼬리 2열 격자(`gap-4`)가 간격을
      //    가져갔었는데 그 격자를 받지 않았으므로 임자가 다시 여기다.
