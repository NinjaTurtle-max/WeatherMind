    ...
    <div className="pt-2">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)] gap-4 lg:grid-cols-2">
        ...
        <div className="contents lg:flex lg:flex-col lg:gap-4">
          ...
          {/* 🔴 **학습 지역은 오른쪽 열이다**(2026-08-19 사용자 지시 — 그림으로
              오른쪽 자리를 지정). 2026-08-11에 왼쪽으로 보냈던 것을 되돌린다:
              그때 이유는 「학습 진도」를 빼서 오른쪽이 짧아진 것이었는데,
              2026-08-12에 지식 단계가 오른쪽으로 들어오면서 그 전제가 사라졌다.
              지금은 왼쪽(프로필+배지)이 더 길고, 그 차이는 배지 칸이 먹는다.
              ⚠️ order-7은 그대로다 — 1열로 쌓이는 좁은 화면에서 배지와 할 일
              사이에 설정이 끼지 않게 하는 값이고, **어느 열에 적혀 있든 이
              숫자가** 순서를 정한다(좁은 화면은 한 픽셀도 안 바뀐다).
              ⚠️ 남는 높이는 이 칸이 **안 먹는다**(`lg:flex-1` 없음) — 한 줄짜리
              카드를 늘이면 그냥 빈 카드가 된다(2026-08-12에 142px까지 늘렸다가
              되돌린 자리). 흡수는 왼쪽 배지 칸이 계속 맡는다. */}
