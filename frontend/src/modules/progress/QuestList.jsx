  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h2 className="text-base font-extrabold text-slate-900">{t('quests.title')}</h2>
        <span className="text-xs font-bold text-slate-400">
          {t('quests.doneCount', { done: doneCount, total: quests.length })}
        </span>
      </div>
      ...
      {hidden > 0 && (
        <p className="mt-2 rounded-xl bg-slate-50 px-3 py-2 text-center text-xs font-medium text-slate-500 ring-1 ring-slate-200">
          {t('quests.moreAfterFirstSession', { count: hidden })}
        </p>
      )}
    </div>
  );
