...
try {
  ...
  //    **아홉 판이 매번 계산은 통과하고 화면에서 틀렸다.** 최종 판정은 사람이 한다.
  {
    ...
    //    갈린 것」이라는 **뜻**으로만 고른다.
    {
      ...
      for (const it of city) {
        // ⚠️ **z까지 키에 넣는다**(2026-08-20 정정). x만 쓰면 도시가 세 줄이 된
        //    순간 **같은 x의 앞줄·뒷줄이 한 기둥으로 합쳐진다** — 8채가 5개로
        //    세어졌다. 합쳐도 이번엔 높이가 균일해 비율은 안 틀렸지만, 줄마다
        //    높이가 갈리는 판이 오면 **조용히 거짓**이 된다.
        const key = `${lo(it, 0).toFixed(3)}~${hi(it, 0).toFixed(3)}|${lo(it, 2).toFixed(3)}~${hi(it, 2).toFixed(3)}`;
    ...
    // ── 7-F2) 🔴 **도시가 줄을 이루는가** ─────────────────────────────────
    // **2026-08-20 클라이언트**: *"건물을 두 줄 또는 3줄로 해서 부자연스러움만
    // 커버하고"*. 지금은 세 줄이다(z 0.09 / 0.20 / 0.31).
    //
    // 🔴 **되돌림 시험에서 세 줄을 한 줄로 뭉개도 아무 계약도 안 울었다.**
    //   「없는 계약은 만들면 된다」 — 여기가 그 자리다.
    // ⚠️ 줄 **수**를 박지 않는다(3을 적으면 네 줄로 늘릴 때 헛운다). 요구는
    //    **「깊이가 갈린 줄이 둘 이상이고 서로 관통하지 않는다」**이다. 관통을 함께
    //    무는 이유: 9차의 「레이어를 쌓아서 부정합」이 바로 깊이 띠가 겹친 상태였다.
    {
      // ⚠️ **지면 위로 선 것만 센다**(2026-08-20 정정). 종전엔 `city` 전부를 셌는데
      //    빗물받이(y -0.044~0.007)·지하(-0.050~-0.002)가 **한 줄로 세어져**,
      //    건물을 전부 한 줄로 뭉개도 「줄 2개」라 **되돌림에서 안 울었다.**
      //    묻는 것은 「도시가 줄을 이루는가」이므로 **땅 위에 선 것**만이 대상이다.
      const rows = new Map();
      const standing = city.filter((it) => lo(it, 1) >= -1e-6);
      for (const it of standing) {
        const key = `${lo(it, 2).toFixed(3)}~${hi(it, 2).toFixed(3)}`;
        if (!rows.has(key)) rows.set(key, [lo(it, 2), hi(it, 2)]);
      }
      const bands = [...rows.values()].sort((a, b) => a[0] - b[0]);
      const crossed = bands.filter((b, i) => i > 0 && b[0] < bands[i - 1][1] - 1e-9);
      check(
        `도시가 깊이로 줄을 이룬다 — 땅 위 물체 ${standing.length}개가 줄 ${bands.length}개 (${bands.map((b) => `${b[0].toFixed(2)}~${b[1].toFixed(2)}`).join(' · ')}), 관통 ${crossed.length}건`,
        bands.length >= 2 && crossed.length === 0,
        bands.length < 2
          ? `도시 물체의 깊이 띠가 ${bands.length}개뿐이다 — 한 줄로 늘어서면 판이 납작해진다(클라이언트 반려 사유).`
          : '깊이 띠가 서로 관통한다 ⇒ 앞뒤가 물체마다 갈려 9차의 「레이어를 쌓아서 부정합」으로 돌아간다.',
      );
    }
  ...
  //    **올리는 것은 답이 아니다**(그때는 표현을 줄이는 것이 답이다).
  {
    ...
    const RATCHET = {
      ...
      //     이기려 하고 있지 않은가?
      //
      // 🔴 **2026-08-20: 18 → 25. 클라이언트 판정으로 올린다** — *"3줄로 가게 래칫
      //   올려"*. 위 문단이 *"다시 올리려는 판이 오면 먼저 의심할 것: 조각을 늘려
      //   무언가를 이기려 하고 있지 않은가?"*라고 적어 뒀고, **그 의심에 답한다:**
      //   아니다. 늘어난 7은 **도시가 두 줄 → 세 줄(5채 → 8채)로 커진 것**이고,
      //   가림·정렬을 이기려고 조각을 쪼갠 것이 하나도 없다. 물은 여전히 세 판이고
      //   물체는 여전히 「수면선에서 둘로 가른 것」뿐이다.
      //   ⚠️ 판정은 클라이언트가 했다. **세션이 스스로 올리지 않는다** — 8/19에
      //     15 → 18로 올릴 때 근거를 적었듯, 이 줄도 근거 없이는 못 움직인다.
      floodSolids: 25,
      floodLabels: 5,
    };
    const measure = (id) => {
