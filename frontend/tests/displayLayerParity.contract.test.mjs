...
try {
  ...
  const HANGUL_GAPS = [
    // ✅ **MT-22 6건이 여기서 사라졌다(2026-08-20 · §4.25 이월 집행).** 종전에는
    // 모식도 3종의 카드 제목·설명·aria, 단계 컨트롤, 장면 라벨·단계 캡션 **72줄**이
    // 「i18n 위생 담당·대회 후」로 걸려 있었다. 전부 `explore.schematic.*`
    // (board.ko.js·board.en.js)로 나갔고 **값은 한 글자도 안 바뀌었다**(SSR 덤프
    // 바이트 동일 확인). 여섯 파일 모두 실측 한국어 **0줄**이라 줄 수를 낮추는 것이
    // 아니라 **행을 지웠다** — 아래 「유령 면제」 검사가 그것을 요구한다.
    //
    // 🔴 **면제가 비었다고 검사가 느슨해진 것이 아니다.** `src/**` 전수를 훑으므로
    // 새 한국어가 한 줄만 들어와도 「미등재」로 즉시 운다.
  ];
  // ⚠️ MT-22의 `src/modules/explore/schematic/**`가 병합되면 이 검사가 그 파일들을
  // 처음으로 본다 — 지금은 여섯 파일 전부 한국어 0줄이라 등재할 것이 없다. 외부화가
  // 다시 밀리는 파일이 생기면 HANGUL_GAPS에 { file, lines, why }를 넣어야 초록이
  // 된다. **그 압력이 의도다**: 고정 목록이었다면 새 디렉터리가 통째로 안 보인 채
  // 들어왔을 것이다.
