...
try {
  ...
  /*
   * 🔴 **정정 4판(2026-08-21 사용자 지시).** 해설이 **다시 격자 밖 전폭**으로
   *    나갔다. 3판 주석이 *"지표가 한 줄로 눕고 왼쪽이 길어지면서 오른쪽에 자리가
   *    남게 됐다"*고 적었는데, 그 전제가 이번에 바뀌었다 — **모식도(C1)가 왼쪽
   *    열에서 오른쪽 열로 넘어오면서** 오른쪽이 더 이상 짧지 않다.
   *
   *    지우지 않고 정정하는 이유는 3판 판단이 그 배치에서는 옳았기 때문이다.
   *    여기 남길 교훈은 두 판이 같다: **「어느 열이 짧은가」는 구성이 바뀌면
   *    뒤집히고, 해설의 자리는 그 값에 딸린 결과지 그 자체로 정해지는 것이 아니다.**
   *
   *    실측(1536)으로 세 배치를 다 재고 골랐다:
   *      가 해설 오른쪽 열   화면 1,496 · 열 여백 0 — 단 CO₂ 카드가 221px를
   *                          흡수해 **410 → 631px**로 부풀었다(카드 안이 휑하다)
   *      나 해설 전폭 맨 밑   화면 1,518 · **오른쪽 열 아래 151px 빔**
   *      다 나 + 지표 세로   화면 1,518 · 오른쪽 여백 **24px** · CO₂ 제 크기 ← 채택
   */
  checkMt21('ⓓ 「왜 그럴까」가 격자 밖 전폭이다(4판) — 열 안으로 되돌리면 CO₂ 카드가 221px 부푼다',
    at("explore.common.whyTitle") > closeGrid);
  checkMt21('ⓓ CTA가 해설보다 아래다 — 설명을 읽고 다음으로 간다',
    at("explore.climate.cta") > at("explore.common.whyTitle"));
  // 왼쪽은 곡선 → 슬라이더 순이다(곡선 위 점을 보며 그 아래를 미는 동선).
  /*
   * ⓓ2 **C1 모식도는 오른쪽 열 맨 위다** (2026-08-21 사용자 지시 — "왼쪽에는
   *     아노말리 그래프·슬라이드, 오른쪽에는 애니메이션 모식도와 해수면상승·폭염일수").
   *
   *     왼쪽 열에서 넘어왔다. 종전 자리의 근거(*"슬라이더 위에 두어 조작 전에 구조를
   *     먼저 읽게 한다"*)는 지금도 살아 있고, 그 「먼저」를 위아래 대신 **좌우**가
   *     맡는다 — 왼쪽은 원인을 만지는 쪽, 오른쪽은 결과를 받는 쪽으로 갈렸다.
   *     ⚠️ 「오른쪽 열 안」만 물면 부족하다. 지표 **아래**로 내려가도 통과하는데,
   *        그러면 조작 전에 구조를 읽는다는 근거가 사라진다 — 앞뒤를 함께 문다.
   */
  checkMt21('ⓓ2 C1 모식도가 격자 안이다', inGrid('<SchematicPanel'));
  checkMt21('ⓓ2 모식도가 CO₂ 슬라이더보다 **뒤**다 — 왼쪽 열로 되돌리면 붉어진다',
    at('<SchematicPanel') > at("explore.climate.co2Label"));
  checkMt21('ⓓ2 모식도가 지표보다 **앞**이다 — 조작 전에 구조를 먼저 읽는다',
    at('<SchematicPanel') < at('<IndicatorCard'));
  ...
  // ⚠️ **흡수자가 하나로 줄었다(4판).** 해설이 격자를 나가면서 오른쪽 열의
  //    `flex-1` 카드도 함께 나갔다 — 그리고 **나가면서 그 클래스를 뗐다**.
  //    전폭 카드에 `flex-1`이 남으면 아무 일도 안 하는 죽은 클래스가 되고,
  //    다음 사람이 그것을 보고 「이 카드는 늘어나는 카드」로 읽는다.
  checkMt21('ⓓ 격자 안 흡수자는 왼쪽 CO₂ 하나다 (해설이 나가며 제 flex-1도 뗐다)',
    (climateSrc.match(/className="flex flex-1 flex-col rounded-2xl/g) ?? []).length === 1);
  ...
  /*
   * 🔴 **정정(4판) — 지표는 넓은 화면에서 세로로 쌓인다.** 3판은 "어느 폭에서나
   *    한 줄"이었고 근거는 「한 줄로 누워야 오른쪽에 자리가 남고 그걸 CO₂가 먹는다」
   *    였다. 4판에서 모식도가 오른쪽으로 오면서 그 자리가 없어졌다 — 오히려
   *    한 줄로 누우면 오른쪽 열 **아래가 151px 빈다**(실측). 세로로 쌓아 24px로 줄였다.
   *    ⚠️ 좁은 화면에서는 **한 줄이 맞다**(2열). 1열로 쌓으면 카드 두 장이 세로로
   *       길어져 스크롤만 는다 — 그래서 `grid-cols-2 lg:grid-cols-1`이다.
   */
  checkMt21('ⓔ 지표 2종이 좁으면 2열 · lg에서 세로 1열이다 (4판)',
    /className="grid grid-cols-2 gap-3 lg:grid-cols-1"/.test(climateSrc));
