...
// ── ⑦ 학습 화면이 홈을 흡수했다 (소스 계약) ────────────────────────────────
{
  ...
  // 사슬의 **첫 노드 y가 섹션마다 같아야** 한다(2026-08-11 사용자 제보: "섹션을
  // 넘길 때 배치가 일정하지 않다"). 그냥 가운데 정렬이면 블록이 칸 수만큼
  // 커졌다 줄었다 하면서 첫 노드가 최대 152px 튄다.
  //
  // 2026-08-12(사용자 제보: "기초과학 탭에서 전부 살짝씩 위로"): 위로 붙이기만
  // 하면 이번엔 **전부 위로 쏠린다** — 기초 과학은 최대 섹션이 3칸인데 --n에는
  // 코스 간 크기를 맞추는 바닥값 4가 깔려 있어 어떤 섹션도 트랙을 못 채운다.
  // 그래서 「사슬 아래에 모자란 칸만큼 여백을 깔고 그 덩어리를 가운데로」다:
  //   · 덩어리 높이 = --nfit(이 코스 최대 칸) 사슬 높이 → **섹션마다 상수**
  //   · 가운데 정렬은 브라우저가 **트랙 실제 높이**로 계산한다(100cqh는 컨테이너
  //     높이라 43px 어긋난다 — 실측하고 이 방식으로 바꿨다)
  // 실측(1440×1000): 3칸 168/168 정중앙 · 2칸도 첫 노드 y=168 → 편차 0.
  ...
  ok(
    ...
    '.wm-vpath가 가운데 정렬이 아니다 — 아래 여백 방식은 center와 짝이다',
  );
  ok(
    /padding-bottom:\s*calc\(var\(--dot\)[^;]*--nfit[^;]*--nsec/.test(vpath),
    '.wm-vpath에 모자란 칸만큼의 아래 여백이 없다 — 짧은 섹션에서 첫 노드가 튄다',
  );
  // 자리 기준은 --nfit(코스 실제 최대)이고 크기 기준은 --n(바닥값 4)이다.
  // 둘을 같은 변수로 합치면 둘 중 하나가 반드시 깨진다(크기 통일 ↔ 위 쏠림).
  ok(
    /'--nfit':\s*fitN/.test(path) && /'--nsec':\s*units\.length/.test(path),
    '경로가 --nfit(코스 최대 칸)·--nsec(이 섹션 칸)을 안 넘긴다',
  );
  // fitN은 ① 이 코스의 실제 최대 칸 수를 보고 ② **상한(sizingN)으로 조인다**.
  // ①이 없으면(바닥값 4를 쓰면) 짧은 코스에서 위 쏠림이 남고, ②가 없으면 시드가
  // 커진 지금 10칸 섹션에 9칸(기상)·25칸(기초과학) 분량의 빈 여백이 붙는다.
  ok(
    /const fitN = Math\.min\(\s*sizingN,\s*Math\.max\(1, \.\.\.withUnits\.map/.test(path),
    'fitN이 sizingN 상한으로 조여지지 않는다 — 예약이 트랙보다 커져 스크롤만 길어진다',
  );
