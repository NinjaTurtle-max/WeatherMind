 ...
 *   WM_FAULT=label-crowd   **라벨의** `until`을 걷어낸다(= 지적된 「글자가 난잡하다」
 *                          그 상태로 되돌린다). 실측 복원치는
 *                          **C1 12 · T1 12 · T2 16**이고 3장면 모두 운다.
 *                          ⚠️ **T1만 원래 값(12)으로 정확히 되돌아간다.** C1은 지적
 *                          당시 15였는데 12까지만 오르고, T2는 오히려 8 → 16으로
 *                          더 커진다 — `until`은 되돌려도 **합쳐 버린 라벨과 캡션으로
 *                          옮긴 라벨은 되돌아오지 않고**(C1: 19·9·6 → 34 합계 ·
 *                          「온실기체 층」 삭제), T2는 단계 전용 라벨(`at = until`)이
 *                          원래부터 많아 그 배타성이 풀리면 전 단계가 겹쳐 쌓이기
 *                          때문이다. 즉 이 주입은 「원래 화면 재현」이 아니라
 *                          **「상한 단정이 공허하지 않다」의 증거**다.
 */
...
try {
  ...
  /**
   * ═══════════════════════════════════════════════════════════════════════════
   *  🔴 단계당 라벨 상한 — **왜 6인가**(2026-08-19 실측으로 정한다)
   * ═══════════════════════════════════════════════════════════════════════════
   * 클라이언트가 갤러리에서 실화면을 보고 지적했다: *"복사수지하고 홍수 태풍들
   * 조금씩만 가독성 높혀줘 **글자들이 너무 난잡하다**"*. 원인을 값으로 특정했다 —
   * **잘리거나 겹치는 것이 아니라 수가 많은 것**이었다(프레임 밖 0건 · 폭 인식
   * 겹침 T1 1 · T2 1 · C1 2). 지적 당시 단계별 라벨 최대는
   * **C1 15 · T1 12 · T2 8**이었다.
   *
   * 상한을 6으로 잡은 근거는 **보드 실측**이다(같은 렌더러·같은 판형 260×150).
   * 2026-08-19 실측: 보드 20장면 79단계, 평균 3.49, 분포
   * `{1:8, 2:17, 3:21, 4:15, 5:9, 6:5, 8:2, 10:2}`.
   * ⚠️ **최댓값 자체는 10이고 그것을 상한으로 쓰지 않는다** — 6을 넘는 3장면이
   * 하필 **같은 지적으로 지금 수리 중인 그 장면들**이기 때문이다:
   *   `wildfire_risk_dry_gale` 10 · `flood_risk_saturated_inflow` 10 ·
   *   `siberian_gale_wildfire` 8
   * (클라이언트 문장의 「홍수」와 산불 장면이다. 결함 집합을 기준으로 삼으면 결함을
   * 계약으로 굳힌다.) **나머지 17장면의 최대가 정확히 6**이고, 그것이 「보드처럼
   * 보이는 화면」의 실측 상한이라 그 값을 그대로 쓴다.
   * 👉 즉 이 값은 취향이 아니라 **「보드와 같아 보이는가」라는 이 과업의 판정 기준을
   *    라벨 수로 옮긴 것**이다. 수리 중인 3장면이 6 이하로 내려오면 이 문단의
   *    예외 목록을 지우고 근거를 「20장면 전건」으로 고칠 것.
   * ⚠️ 값을 올리고 싶어지면 **라벨을 캡션(`STEPS[].note`)으로 옮기는 쪽**이 먼저다 —
   *    이 상한을 올리는 것은 지적을 되돌리는 일이다.
   */
  const LABEL_CAP = 6;
  ...
  for (const [name, scene, steps] of SCENES) {
    ...
    // 🔴 단계당 라벨 **수** — 위 검사가 못 보던 자리다. 종전 판은 「프레임 안인가」와
    //    「겹치는가」만 물었고 **둘 다 초록인데 화면이 반려됐다**(C1 15개 · 전건
    //    프레임 안 · 겹침 2). 그래서 수를 직접 문다. 근거는 위 `LABEL_CAP` 문단.
    {
      // 고장 주입: 라벨의 `until`을 걷어낸다 = 지적 당시 화면으로 되돌린 관측
      const crowded = FAULT === 'label-crowd'
        ? { ...scene, items: scene.items.map((it) => (it.type === 'label' ? { ...it, until: undefined } : it)) }
        : scene;
      const perStep = steps.map((_, s) => labelsFor(crowded, s, 260 / 150).length);
      const over = perStep
        .map((n, s) => ({ n, s }))
        .filter(({ n }) => n > LABEL_CAP);
      check(`${name} — 단계마다 캔버스 라벨이 ${LABEL_CAP}개 이하다(보드 실측 상한)`,
        over.length === 0,
        `단계별 ${perStep.join(',')} — 상한 ${LABEL_CAP} 초과: `
        + `${over.map(({ n, s }) => `step${s} ${n}개`).join(' · ')}`
        + ` [${over.map(({ s }) => labelsFor(crowded, s, 260 / 150).map((l) => l.text).join('/')).join(' || ')}]`
        + ' — 「글자들이 너무 난잡하다」가 이 수다. 상한을 올리지 말고 긴 문구를'
        + ' STEPS[].note(캔버스 밖 캡션)로 옮기거나 역할이 끝난 라벨에 until을 줄 것');
    }
  }
