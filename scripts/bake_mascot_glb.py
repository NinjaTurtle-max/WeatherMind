...
대신 런타임이 할 일을 전부 빌드타임으로 옮긴다:
  ...
  ⓒ 재질별 병합          — 드로우콜 44(프리미티브) → **4**(재질 5 중 얼굴판 제외.
                          왜 4인지는 아래 SKIP_MATERIALS가 소유한다)
...
# (2026-08-13 실측) 몸통 48 · 얼굴 620에서 삼각형 94,708 → **27,011** ·
# 파일 **272,998 B**. ⚠️ 이 줄에 한때 "28,683 · 291,430 B"가 적혀 있었는데 그것은
# 아래 SKIP_MATERIALS(얼굴판 제외) 이전 값이라 같은 날 낡았다 — 값을 바꾸면 다시
# 재서 여기 적을 것(CLAUDE.md §0-1).
