...
FRONT_TESTS=(explore explore-goals session session-blocks entry-flow placement placement-skip visual gating board-entry assist webgl schematic overlay display-parity i18n ui-copy course-select guest-convert review-queue region learn-path home home-entry mascot duel hint-character session-retry detective knowledge-level onboarding-save guide-bot guide-bot-3d session-expiry)
