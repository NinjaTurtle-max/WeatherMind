...
FRONT_TESTS=(explore explore-goals session session-blocks entry-flow placement visual gating board-entry assist webgl overlay display-parity i18n ui-copy course-select guest-convert review-queue region learn-path home home-entry mascot duel hint-character session-retry detective hindcast knowledge-level onboarding-save guide-bot guide-bot-3d session-expiry)
