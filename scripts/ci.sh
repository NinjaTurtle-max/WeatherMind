...
FRONT_TESTS=(explore explore-goals session session-blocks placement visual gating board-entry assist webgl overlay i18n ui-copy course-select guest-convert review-queue region learn-path home home-entry mascot duel hint-character session-retry detective knowledge-level onboarding-save)
