...
# 문항을 통째로 못 본다:
#   · **반대** — 「두 번째 선택지는 방향이 **반대**다」(수리 전 [575]). 그 한 건이
#     **정답 자리를 오답이라 가리키는 진짜 결함**이었고(⑥ⓐ의 존재 이유) 이 낱말
#     없이는 미탐이었다. 그 문항은 PR #109에서 수리됐지만 **어휘는 남긴다** —
#     같은 말로 쓰는 다음 저작을 막는 것이 이 낱말의 몫이다.
#   · **어긋** — 「… 점에서 **어긋난다**」 2건. 둘 다 오답 자리를 가리켜 적발 0이지만,
#     같은 주장을 쓰는 어휘를 목록 밖에 두면 다음 저작에서 새어난다.
# 둘 다 **wrong 방향**이라 「오답 자리를 정답이라」 오탐을 만들 수 없다 — 정답 자리를
# 가리킬 때만 운다. 실측 비용 0(2026-08-18 · 시드 1,018건 · ⑥ⓐ 적발 0건).
