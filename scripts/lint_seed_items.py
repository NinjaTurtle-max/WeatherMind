...
# ⚠️ **2026-08-18 2차 리뷰: 두 낱말을 실측 근거로 더한다.** 목록은 짧게 유지하되,
# 「이 선지는 틀렸다」를 실제로 그렇게 쓴 본시드 어휘가 빠져 있으면 게이트가 그
# 문항을 통째로 못 본다(시드 1,012건 실측):
#   · **반대** — 「두 번째 선택지는 방향이 **반대**다」. 이 한 건이 **정답 자리를
#     오답이라 가리키는 진짜 결함**이고(⑥ⓐ의 존재 이유) 이 낱말 없이는 미탐이었다.
#   · **어긋** — 「… 점에서 **어긋난다**」 2건. 둘 다 오답 자리를 가리켜 지금은
#     적발 0이지만, 같은 주장을 쓰는 어휘를 목록 밖에 두면 다음 저작에서 새어난다.
# 둘 다 **wrong 방향**이라 「오답 자리를 정답이라」 오탐을 만들 수 없다 — 정답 자리를
# 가리킬 때만 운다.
GATE_WRONG_CONTEXT: tuple[str, ...] = (
    "오답", "틀리", "틀린", "오독", "잘못", "혼동", "헷갈", "어긋", "반대",
)
# ⚠️ **어간 「옳지」·「맞지」가 없었다**(2026-08-18 리뷰 부수 지적): 「옳지 않다」는
# 종결형(옳다·옳은)에 안 걸려 극성 없음(None)이 되고, 정답 자리를 가리켜도 빠졌다.
GATE_RIGHT_CONTEXT: tuple[str, ...] = (
    "정답", "옳다", "옳은", "옳지", "맞다", "맞는", "맞지",
)
# 「정답이 **아니다**」·「옳지 **않다**」·「정답이 될 수 **없다**」 — 긍정어가 부정되면
# 뜻이 뒤집힌다. ⚠️ 이것이 없을 때 「두 번째 선지는 **정답이 아니다**」(맞는 해설)가
# 「오답 자리를 정답이라 가리킨다」로 **탈락**했다(실행 재현). 부정은 긍정어 **뒤쪽**에 온다.
# ⚠️ **「아님」은 「아니」의 부분문자열이 아니다**(아 + 님 ≠ 아 + 니) — 활용형을 따로 적는다.
_NEGATORS: tuple[str, ...] = ("아니", "아님", "아닌", "않", "없")
# 긍정어 뒤로 이만큼 안에 부정어가 오면 부정으로 본다.
# ⚠️ 종전 6은 「정답이 될 수 **없다**」를 놓쳤다(부정어가 7번째 글자다). 10으로
# 넓히는 대가는 **문장 경계에서 자르는 것으로** 갚는다 — 안 자르면 「… 정답이다.
# 나머지는 아니다」의 뒷문장 부정어가 앞 문장의 주장을 뒤집는다.
_NEGATION_SPAN = 10
...
def claim_polarity(window: str) -> str | None:
    ...
    **극성어를 전부 모아 창 안 마지막 것을 고른다.** 한국어는 서술어가 절 끝에 오므로
    서수(주제)에 대한 주장은 마지막 극성어다 — 「세 번째 선지는 **정답처럼 보이지만**
    실제로는 **오답이다**」에서 주장은 「오답」이다.

    ⚠️ **종전 구현이 세 가지를 동시에 틀렸다**(2026-08-18 리뷰 결함 1·3):
      ⑴ 창 안 **첫 긍정어**만 보고 `GATE_WRONG_CONTEXT`를 **아예 안 봤다** — 위
         문장이 「오답 자리를 정답이라 가리킨다」로 **탈락**했다(맞는 해설을 규탄).
      ⑵ `while index != -1`이 **한 번도 반복하지 못하는 죽은 루프**였다(모든 경로가
         return이고 `index`가 재계산되지 않았다). 그래서 두 번째 긍정어는 원리적으로
         못 봤다.
      ⑶ 부정 창이 6자라 「정답이 될 수 **없다**」가 밖으로 나갔다.

    ⚠️ **버린 대안을 적어 둔다** — 리뷰가 권고한 「연결 어미로 앞 절의 극성어를 걸러낸
    뒤 **앵커에 가장 가까운 것**이 이긴다」는 실행해 보고 버렸다: PM 함정
    (「두 번째 선지는 정답이고 세 번째 선지는 오답이다」)은 통과하지만 결함 1의 재현
    **두 개**(「정답처럼 보이지만」·「정답과 혼동하기 쉬운」)에서 오탐이 살아남는다.
    최근접은 둘 다에서 「정답」을 고른다. 26종 픽스처 + 실시드 대조로 갈랐다.

    **남는 오탐 형태**(둘 다 2026-08-18 시드 1,012건에 **0건** — 코드로 막지 않고 적어 둔다):
      · 「N번째 선지는 오답이고 나머지가 정답이다」 — 뒤에 서수도 문장 끝도 없어
        창이 안 끊기고, 마지막 극성어가 앞 절의 것이 아닌데도 이긴다.
      · 「정답임에 틀림없다」 — 「없」이 부정으로 읽혀 right가 wrong으로 뒤집힌다.
    """
    marks: list[tuple[int, str]] = []
    for word in GATE_RIGHT_CONTEXT:
        for match in re.finditer(re.escape(word), window):
            tail = window[match.end() : match.end() + _NEGATION_SPAN]
            cut = _SENTENCE_END.search(tail)
            if cut:
                tail = tail[: cut.start()]   # 부정은 **같은 문장** 안에서만 본다
            marks.append(
                (match.start(), "wrong" if any(n in tail for n in _NEGATORS) else "right")
            )
    for word in GATE_WRONG_CONTEXT:
        marks += [(m.start(), "wrong") for m in re.finditer(re.escape(word), window)]
    if not marks:
        return None
...
def hint_ordinal_errors(item: dict) -> list[tuple[str, str]]:
    ...
    (2026-08-18 정정). 재실측: **시드 1,012건 · mc 310건 기준으로 명사 없는 서수만
    있는 해설 3건**(「선택지」를 명사에 넣은 뒤 값이다 — 그전에는 4건이었고 그 1건이
    실결함이었다). 3건 중 **1건**(「두 번째 몫」)이 선지와 무관한 정상 서술이고 **2건**은
    실제 선지 참조라 **의도된 미탐**이다(그 맞바꿈은
